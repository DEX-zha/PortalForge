...
### `igz script <decoded> <record offset> --fixups <map.json> [--limit n]` / `igz scripts <decoded> --fixups <map.json>`
Type-92 script decoding: `script` lists a script`s instruction-pointer array in order (index, record, opcode class, opcode text, string/float/triple arguments, references to variables or other instructions; shared targets outside the blob are arguments by reference); `scripts` audits the owner model on every type-92 record (count == capacity, size word 0x80000000|count*4, array at +0x34, blob fully tiled). On the tutorial: 157/157 scripts pass; 287 distinct opcodes; the CONFIRMED spawn record is InitializeLevel.ai #2 `clone||at|facing||cloned` (finding `igz.script.clone-opcode`).

### `igz clone-entity <decoded> <P> --end <P end> --fixups <map.json> --finding <id> --replace-node next|<hex victim> [--link-field 0x64] --out <decoded> [--plan <plan.json>]`
Zero-shift duplication — the path that instantiated the first duplicate (run `m3-…-1789301273731`). The bytes of a same-size, same-type chain node (by default `P`'s successor) are replaced by a copy of `P`; the copy keeps the victim's `next` and gets refcount 1, so the chain reads `pred -> P -> copy -> victim.next`. Exactly one block changes (6 words for a 0x70 placement), no record moves, no table/count/size is touched: this is what the count-bounded head walk requires. Refuses non-CONFIRMED findings; VALID only if the file length is unchanged, nothing outside the victim block changed, the copy is recognised and the chain resolves. The victim record is lost.
