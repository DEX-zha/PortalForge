...
- Gates: `node tools/ssa-archive/cli.mjs gates` prints M0..M5 from `docs/m*-status.json`. M0, M1 and M2 are PASS (2026-09-12), so size-preserving edits of CONFIRMED properties are permitted (FR-016). M3 is FAIL (2026-09-13): an appended, listed clone loads but is not instantiated; structural editing waits until the instantiation structure is found (feature 002, `docs/m3-status.json`). Geometry, collision and gameplay changes wait for M4A, M4B and M5. The constitution (`.specify/memory/constitution.md`) records these invariants.
