...
test('png: encode then decode round-trips pixels, crop zooms, diffBoxes finds the changed region', () => {
  ...
  const fa = encode(a, path.join(dir, 'a.png')),
    fb = encode(b, path.join(dir, 'b.png'));
  const da = decode(fa),
    db = decode(fb);
  assert.equal(da.w, 40);
  assert.equal(da.h, 30);
  assert.equal(da.ch, 3);
  ...
  assert.equal(z.w, 30);
  assert.equal(z.h, 24);
  assert.equal(z.data[0], 240);
  assert.equal(z.data[1], 20); // the zoomed crop starts on the patch
  const clipped = crop(db, 35, 25, 20, 20, 1); // a region running off the edge is clipped
  assert.equal(clipped.w, 5);
  assert.equal(clipped.h, 5);
