...
test('patch workspace contains only differing files, a valid Riivolution XML and a forward-slash descriptor', () => {
  ...
  const game = path.join(dir, 'fake.wbfs'); fs.writeFileSync(game, Buffer.alloc(16));
  const orig1 = path.join(dir, 'orig1.arc'); fs.writeFileSync(orig1, Buffer.alloc(3000, 1));
  const repl1 = path.join(dir, 'repl1.arc'); fs.writeFileSync(repl1, Buffer.alloc(61447, 2));
  const orig2 = path.join(dir, 'orig2.bld'); fs.writeFileSync(orig2, Buffer.alloc(500, 3));
  const same2 = path.join(dir, 'same2.bld'); fs.writeFileSync(same2, Buffer.alloc(500, 3));
  const out = path.join(dir, 'patch');
  const ws = buildPatchWorkspace({ experimentId: 'm1-test', game,
    replacements: [{ disc_path: 'level/Level_027_Tutorial.arc', file: repl1, original: orig1 }, { disc_path: 'level/Level_027_Tutorial.bld', file: same2, original: orig2 }], outDir: out });
  ...
  assert.match(xml, /<file disc="\/level\/Level_027_Tutorial.arc" external="\/files\/level\/Level_027_Tutorial.arc" resize="true" create="false" \/>/);
  assert.doesNotMatch(xml, /Level_027_Tutorial\.bld/);
  const d = JSON.parse(fs.readFileSync(ws.descriptor, 'utf8'));
  for (const p of [d['base-file'], d.riivolution.patches[0].xml, d.riivolution.patches[0].root]) assert.ok(!p.includes('\\'), p);
