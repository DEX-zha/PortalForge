...
test('file-monitor size parsing handles thin-space thousands separators and single digits', () => {
  assert.equal(GameSession.monitorSize('07:52:396 Core\\HW\\DVD\\FileMonitor.cpp:86 W[FileMon]:  52 733 kB level/Level_027_Tutorial.arc'), '52733 kB');
  assert.equal(GameSession.normSize('52 735 kB'), GameSession.normSize('52735 kB'));
  assert.equal(GameSession.monitorSize('58:22:981 Core\\HW\\DVD\\FileMonitor.cpp:86 W[FileMon]:       0 kB hbm/config.txt'), '0 kB');
...
test('both timeout flavours are recognised so loading stalls are tolerated, other errors are not', () => {
  assert.equal(GameSession.isTimeout(new Error('dolphin_frame_advance: frame_advance(120) timed out after 15000ms')), true);
  assert.equal(GameSession.isTimeout(new Error('Dolphin bridge timeout: start a game and unpause the emulator.')), true);
...
test('the default level-entry script is well formed and never uses Plus (pause)', () => {
  ...
  for (const s of steps) {
    const keys = Object.keys(s).filter(k => ['press', 'nunchuk', 'wait', 'wait_monitor', 'shot', 'figure', 'save_state'].includes(k));
