...
test('eight additions fit the shared Gecko budget for every static/scripted mixture', () => {
  for (let scripts = 0; scripts <= 8; scripts++) {
    const rows = Array.from({ length: 8 }, (_, i) => ({
      id: -i - 1,
      source: i < scripts ? 2390540 : 3446244,
      model: i < scripts ? 1214412 : 3446580,
      script: i < scripts ? 2348068 : null,
      position: [90 + i, 10, 40],
      heading: 0,
      scale: 100,
    }));
    const compiled = compileNativePatch(rows);
    assert.equal(compiled.count, 8);
    assert.ok(compiled.bytes <= 3256);
...
test('script lifecycle distinguishes earlier creation from final survival without relaxing static checks', async () => {
  const scripted = { id: -1, script: 100 },
    statik = { id: -2 };
  const earlier = [{ verified: true, objects: [{ id: -1 }, { id: -2 }], capture: 'early.png' }];
  const verify = async rows => {
    if (rows.some(a => a.script)) throw Error('actor no longer present');
    return { verified: true, objects: [{ id: -2 }] };
  };
  const result = await verifyNativeLifecycle([scripted, statik], earlier, verify);
  assert.equal(result.lifecycle, 'changed_after_creation');
  assert.equal(result.final_verified, false);
  await assert.rejects(verifyNativeLifecycle([scripted], [], verify), /no longer/);
  await assert.rejects(
    verifyNativeLifecycle([scripted, statik], earlier, async () => {
      throw Error('static missing');
    }),
    /static missing/,
  );
});
test('fresh scene interpretation never reads native addition identities as IGZ offsets', () => {
  ...
  buffer.writeUInt32BE(104, 0);
  buffer.writeUInt32BE(1, 0x54);
...
function fixture(t) {
  ...
  source.offset = 3446244;
  source.model.path = 'models/plant_sunflower_whole.mdl';
  source.model.offset = 3446580;
  ...
  return {
    ...
    dir,
    source,
    add: position => applyEdit(s, { kind: 'add', source: source.offset, position: position ?? [91, 10, 43] }),
  };
}
test('native addition preserves bytes and originals; identities, transforms, undo/redo/reset are independent', t => {
  ...
  const a = add().placement,
    b = add([89, 10, 43]).placement;
  assert.equal(a.offset, -1);
  assert.equal(b.offset, -2);
  assert.equal(s.placements.length, count + 2);
  applyEdit(s, { kind: 'transform', target: a.offset, position: [92, 11, 44], heading: 240 });
  assert.deepEqual(source, snapshot);
  assert.deepEqual(s.buffer, original);
  assert.equal(authorisedWords(s).size, 0);
  undo(s);
  assert.deepEqual(s.additions[0].position, [91, 10, 43]);
  redo(s);
  assert.equal(s.additions[0].heading, 240);
  resetScene(s);
  assert.equal(s.additions.length, 0);
  assert.equal(s.placements.length, count);
  assert.deepEqual(s.buffer, original);
  redo(s);
  redo(s);
  redo(s);
  assert.equal(s.additions.length, 2);
  assert.equal(s.additions[0].heading, 240);
...
test('additions save beside unchanged IGZ; reopen, stale patch and tampered sidecar are checked', async t => {
  const { s, dir, add } = fixture(t);
  add();
  assert.equal(buildSavePlan(s).changes.length, 0);
  const target = path.join(dir, 'saved.decoded'),
    result = save(s, { out: target });
  assert.ok(result.written);
  assert.deepEqual(fs.readFileSync(target), s.buffer);
  const restored = fixture(t).s;
  restored.file = target;
  loadAdditionSidecar(restored);
  assert.deepEqual(restored.additions, s.additions);
  assert.ok(restored.lastSave);
  const deps = { build: () => ({ dir, replacements: [] }) };
  patch(s, { deps });
  ...
  save(s, { out: target });
  const d = JSON.parse(fs.readFileSync(target + '.portalforge.json'));
  d.additions[0].heading = 30;
  fs.writeFileSync(target + '.portalforge.json', JSON.stringify(d));
  assert.throws(
    () => patch(s, { deps }),
    e => e.error === 'STALE_ADDITIONS',
  );
  d.base_sha256 = 'wrong';
  fs.writeFileSync(target + '.portalforge.json', JSON.stringify(d));
  assert.throws(() => loadAdditionSidecar(restored), /does not match/);
});
...
test('full addition capacity survives save/reopen and complete undo/redo/reset without touching original bytes', t => {
  ...
  const snapshot = structuredClone(s.additions),
    file = path.join(dir, 'full.decoded');
  save(s, { out: file });
  const fresh = fixture(t).s;
  fresh.file = file;
  loadAdditionSidecar(fresh);
  assert.deepEqual(fresh.additions, snapshot);
  for (let i = 0; i < NATIVE_LIMIT; i++) undo(s);
  assert.equal(s.additions.length, 0);
  for (let i = 0; i < NATIVE_LIMIT; i++) redo(s);
  assert.deepEqual(s.additions, snapshot);
  resetScene(s);
  assert.equal(s.additions.length, 0);
  assert.deepEqual(s.buffer, original);
  for (let i = 0; i < NATIVE_LIMIT; i++) redo(s);
...
for (const [offset, model, modelPath, script, scriptPath] of [
  ...
  test(`confirmed source ${offset} retains its script and rejects tampered sidecars`, t => {
    ...
    source.model = { offset: model, path: 'Objects/' + modelPath };
    ...
    const target = path.join(dir, 'scripted.decoded');
    save(s, { out: target });
    const document = JSON.parse(fs.readFileSync(target + '.portalforge.json'));
    document.additions[0].script = script + 4;
    fs.writeFileSync(target + '.portalforge.json', JSON.stringify(document));
    s.file = target;
...
test('unsupported templates, nonfinite transforms, scales, duplicate IDs and capacity are refused', t => {
  ...
  // Past the proven slot layout the scene compiles as the compact table; the Gecko budget is the only ceiling.
...
test('native profile install restores exact files and refuses concurrent use or existing Gecko codes', async t => {
  const { s, dir, add } = fixture(t);
  add();
  const code = compileNativePatch(s.additions).ini,
    file = path.join(dir, 'patch.ini');
  fs.writeFileSync(file, code);
  const native = { file, sha256: hash(code), additions: s.additions },
    directory = path.join(dir, 'profile');
  fs.mkdirSync(path.join(directory, 'Config'), { recursive: true });
  const config = path.join(directory, 'Config/Dolphin.ini');
  ...
  assert.equal(fs.existsSync(path.join(directory, 'GameSettings/SSPP52.ini')), false);
  fs.writeFileSync(path.join(directory, 'GameSettings/SSPP52.ini'), '[Gecko]\n$Existing\n');
  await assert.rejects(installNativePatch(native, { directory, occupied: async () => false }), /already contains/);
  assert.deepEqual(fs.readFileSync(config), before);
});
test('runtime creation proof refuses absent recipes and a completed flag with null result', async () => {
  const a = { id: -1, source: 3446244 },
    memory = Buffer.alloc(0x1800);
  await assert.rejects(
    verifyNativeInstances([a], async () => memory),
    /not uniquely consumed/,
  );
  memory.writeUInt32BE(0x50464e41, 0);
  memory.writeUInt32BE(2, 4);
  memory.writeInt32BE(-1, 0x1c);
  memory.writeUInt32BE(0x80dbc020 + a.source, 0x2c);
  await assert.rejects(
    verifyNativeInstances([a], async () => memory),
    /did not complete/,
  );
...
test('scripted creation verifies initial placement while allowing live AI movement and rejects shared state', async () => {
  const base = 0x80dbc020,
    a = { id: -1, source: 2390540, model: 1214412, script: 2348068, position: [90, 10.5, 48], heading: 0 };
  const memory = Buffer.alloc(0x1800),
    copy = Buffer.alloc(0xf8),
    source = Buffer.alloc(0xf8),
    pointer = 0x81220000;
  memory.writeUInt32BE(0x50464e41, 0);
  memory.writeUInt32BE(2, 4);
  memory.writeUInt32BE(pointer, 8);
  memory.writeInt32BE(a.id, 0x1c);
  memory.writeUInt32BE(base + a.source, 0x2c);
  for (const b of [copy, source]) {
    b.writeUInt32BE(0x80481674, 0);
    b.writeUInt32BE(base + a.script, 0xa8);
  }
  copy.writeUInt32BE(1, 0x54);
  copy.writeUInt32BE(base + a.source, 0x5c);
  copy.writeUInt32BE(base + a.model, 0xdc);
  copy.writeUInt32BE(0x81230000, 0xf4);
  copy.writeUInt32BE(0x81240000, 0xe0);
  source.writeUInt32BE(0x81250000, 0xe0);
  a.position.forEach((v, i) => copy.writeFloatBE(v, 0x24 + i * 4));
  [93, 11, 44].forEach((v, i) => copy.writeFloatBE(v, 0x3c + i * 4));
  copy.writeFloatBE(80, 0x4c);
  ...
  assert.deepEqual(result.objects[0].position, a.position);
  assert.deepEqual(result.objects[0].current_position, [93, 11, 44]);
  copy.writeFloatBE(89, 0x24);
  await assert.rejects(verifyNativeInstances([a], read), /does not match/);
  copy.writeFloatBE(90, 0x24);
  copy.writeUInt32BE(0x81250000, 0xe0);
  await assert.rejects(verifyNativeInstances([a], read), /parameters are not independent/);
  copy.writeUInt32BE(0x81240000, 0xe0);
  copy.writeUInt32BE(base + a.script + 4, 0xa8);
  await assert.rejects(verifyNativeInstances([a], read), /script was not retained/);
  copy.writeUInt32BE(base + a.script, 0xa8);
  source.writeUInt32BE(0x81260000, 0xb0);
  copy.writeUInt32BE(0x81260000, 0xb0);
  await assert.rejects(verifyNativeInstances([a], read), /variables are shared/);
  copy.writeUInt32BE(0x81270000, 0xb0);
  assert.equal((await verifyNativeInstances([a], read)).verified, true);
});
