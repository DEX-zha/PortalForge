...
function fixture() {
  ...
  const fixups = {
    ...
    pointer_words: [a + 0x14, b + 0x14, c + 0x20],
    head_pointer_words: [],
    id_words: [],
    cross_pointer_words: [],
  };
...
test('probeFlat and probeBitmap run and score without matching random sections', () => {
  ...
  for (const x of r) {
    assert.ok(x.precision >= 0 && x.precision <= 1);
    assert.ok(x.recall >= 0 && x.recall <= 1);
  }
...
test('decodeScript reads a type-92 script: count/capacity/size word, array at +0x34, instruction opcodes and blob coverage', async () => {
  ...
  const built = buildIgz({
    types: ['metaobject', 'a', 'b', 'c', 'd'].concat(Array.from({ length: 88 }, (_, i) => 't' + i)).concat(['script']),
    strings: ['x', 'set value||=', 'clone||at|facing||cloned'],
    objects: [
      {
        type: 92,
        size: 0x3c,
  ...
  const secOff = built.sections.s1;
  ...
  assert.equal(d.instructions[0].target, i1);
  assert.equal(d.instructions[0].opcode, 'set value||=');
  assert.equal(d.instructions[1].target, i2);
...
test('listPlacements lists type-104 header-table records with position, heading, scale, model, script and layer names', async () => {
  ...
  const types = Array.from({ length: 105 }, (_, i) =>
    i === 52 ? 'layer' : i === 64 ? 'model' : i === 92 ? 'script' : i === 104 ? 'placement' : 't' + i,
  );
  const built = buildIgz({
    types,
    strings: ['Crate_01', 'Props', 'C:/tfb/Content/Models/Objects/crate.mdl', 'Crate.ai'],
    objects: [
      {
        type: 104,
        size: 0x100,
      ...
      // two more placements: a class is only believed to hold placements once three of its records look like one
      {
        type: 104,
        size: 0x100,
        fields: [
  ...
  assert.equal(res.total_type104, 3);
  assert.equal(res.listed, 3);
  const r = res.rows[0];
  assert.equal(r.offset, P);
  assert.equal(r.name, 'Crate_01');
  assert.deepEqual(r.position, [12.5, 3, -7]);
  assert.equal(r.heading, 90);
  assert.equal(r.scale, 100);
  assert.equal(r.model.offset, M);
  assert.match(r.model.path, /crate\.mdl$/);
  assert.equal(r.script.offset, S);
  assert.deepEqual(r.layers, ['Props']);
  assert.equal(listPlacements(built.buf, g, fixups, { layer: 'nothing' }).listed, 0);
  assert.equal(res.placement_type, 104, 'the class is detected, not assumed');
