...
import { lzmaEncodeChunk, lzmaDecode, LZMA_PROPS, encodeEntryChunks, decodeStoredEntry, entryChunkPlan, decodeWorkspace, reencodeEntry } from '../src/iga/decode.mjs';
...
const SKIP = { skip: fs.existsSync(gryphon) ? false : 'fixture character/001_Gryphon.bld not extracted under .local/samples' };

const text = n => { const words = ['igObjectList', 'igTextureAttr2', 'transform', 'entity', 'level']; let s = ''; while (s.length < n) s += words[s.length % words.length] + ' '; return Buffer.from(s.slice(0, n), 'latin1'); };
...
test('multi-chunk entry: encoded chunks decode back; raw fallback for incompressible data', async () => {
  ...
  const blocks = []; let seed = Buffer.from('portalforge-m2-probe');
  while (blocks.reduce((n, b) => n + b.length, 0) < 40000) { seed = createHash('sha256').update(seed).digest(); blocks.push(seed); }
  const random = Buffer.concat(blocks).subarray(0, 40000);
  const raw = encodeEntryChunks(random);
  assert.ok(raw.values.slice(0, 2).some(v => (v & 0x8000) === 0), 'at least one raw chunk expected');
  const back2 = decodeStoredEntry(raw.bytes, { index: 0, size: random.length, chunk_table_index: 0 }, { values: raw.values, word_24: raw.values.length, word_28: 0 });
  assert.equal(Buffer.compare(back2, random), 0);
});

test('single-chunk entry record [0x80][units] is interpreted from the record area', () => {
  const plan = entryChunkPlan({ index: 4, size: 4272, chunk_table_index: 0 }, { values: [0x8000, 0x148, 0x0180, 0x0180], word_24: 2, word_28: 4 });
...
test('fixture: every compressed entry of 001_Gryphon.bld decodes to an IGZ file of the declared size', SKIP, async () => {
...
test('fixture: re-encoding a mutated single-chunk entry yields a VALID archive whose entry decodes to the mutation', SKIP, async () => {
  ...
  const mutated = Buffer.from(data); mutated[mutated.length - 1] ^= 0x01;   // last byte, keeps IGZ header intact
  const repl = path.join(dir, 'mutated.bin'); fs.writeFileSync(repl, mutated);
  ...
  const back = decodeStoredEntry(stored, e, { values: p.chunk_area.values, word_24: p.header.word_24, word_28: p.header.word_28 });
  ...
  const lvlBack = decodeStoredEntry(r.buffer.subarray(lvl.start, lvl.start + lvl.stored_size), lvl, { values: p.chunk_area.values, word_24: p.header.word_24, word_28: p.header.word_28 });
