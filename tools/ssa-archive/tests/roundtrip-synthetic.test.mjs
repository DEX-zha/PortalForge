...
const structural = buf => {
  const p = parseArchive(buf);
  return JSON.stringify({
    h: p.header.header_words,
    hashes: p.hashes,
    entries: p.entries.map(e => [e.name, e.hash, e.start, e.size, e.mode]),
    names: p.name_table.offsets,
  });
};
...
test('same-size replacement changes only the targeted entry bytes and keeps offsets', () => {
  ...
  const manifest = extractToWorkspace(buf, {
    discPath: 'test/d.arc',
    sourceFile: path.join(dir, 'd.arc'),
    outDir: dir,
  });
  ...
  mutated[10] ^= 0x5a;
  const repl = path.join(dir, 'repl.bin');
  ...
  let diffs = 0;
  for (let i = 0; i < buf.length; i++) if (buf[i] !== r.buffer[i]) diffs++;
...
test('different-size replacement relayouts entries on 0x800 boundaries and stays VALID', () => {
  ...
  const manifest = extractToWorkspace(buf, {
    discPath: 'test/e.arc',
    sourceFile: path.join(dir, 'e.arc'),
    outDir: dir,
  });
  const repl = path.join(dir, 'big.bin');
  fs.writeFileSync(repl, Buffer.alloc(manifest.entries[1].size + 5000, 0xab));
...
test('sequential layout with padding is byte-different, structurally equivalent and VALID', () => {
  ...
  const a = parseArchive(buf),
    b = parseArchive(r.buffer);
  assert.deepEqual(
    b.entries.map(e => [e.name, e.hash, e.size, e.mode]),
    a.entries.map(e => [e.name, e.hash, e.size, e.mode]),
  );
  b.entries.forEach((e, i) =>
    assert.equal(
      Buffer.compare(
        r.buffer.subarray(e.start, e.start + e.size),
        buf.subarray(a.entries[i].start, a.entries[i].start + a.entries[i].size),
      ),
      0,
    ),
  );
...
test('replacing a compressed entry without a confirmed re-encoder is refused explicitly', () => {
  ...
  const repl = path.join(dir, 'x.bin');
  fs.writeFileSync(repl, Buffer.alloc(10));
