...
const unit = (descriptor, kind = 'interleaved', frac = 6) => ({
  descriptor, kind, frac, count: 3, vertices: new Float32Array([90, 6, 40, 94, 6, 40, 90, 6, 44]),
...
test('scenery partitions decoded geometry without duplicating shared or already placed world units', () => {
  ...
  const a = { byModel: new Map([[10, [0]], [11, [0]]]) };
  const p = partitionGeometry(units, a, { maxVertices: 3 });
  assert.equal(p.assigned_unique, 1);
  assert.deepEqual(p.scenery.chunks.flatMap(c => c.descriptors), [1, 4]);
  assert.deepEqual(p.unresolved.map(u => u.descriptor), [2, 3]);
...
test('large scenery surfaces are batched separately so backgrounds can be wireframe without hiding terrain', () => {
  const huge = unit(8);
  huge.vertices[0] = -500; huge.vertices[3] = 500;
  const p = partitionGeometry([unit(7), huge, unit(9)], { byModel: new Map() }, { largeSurfaceLimit: 100 });
  const solid = p.scenery.chunks.filter(c => !c.large_surface), large = p.scenery.chunks.filter(c => c.large_surface);
  assert.deepEqual(solid.flatMap(c => c.descriptors), [7, 9]);
  assert.deepEqual(large.flatMap(c => c.descriptors), [8]);
  const material = new THREE.MeshLambertMaterial(), wire = new THREE.MeshLambertMaterial({ wireframe: true });
  ...
  disposeScenery(group); material.dispose(); wire.dispose();
});
...
for (const [name, folder, world, unresolved, excluded] of [
  ...
  const file = fileURLToPath(new URL(`../../../.local/workspaces/${folder}/entries/3-level.bld.decoded`, import.meta.url));
  test(`scenery ${name}: serves missing terrain, excludes UI and preserves edits`, { skip: !fs.existsSync(file) && 'local sample absent' }, () => {
    const s = openSession(file, { deps: { gates: () => ({ status: 'PASS' }) } });
    const original = Buffer.from(s.buffer);
    const result = modelMeshes(s), payload = meshesPayload(s), view = decodeMeshPayload(payload);
    ...
    if (name === 'tutorial') {
      ...
      const target = s.placements.find(p => p.name === 'sunflower(2)') ?? s.placements.find(p => p.model?.path?.endsWith('plant_sunflower_whole.mdl'));
      assert.ok(target);
