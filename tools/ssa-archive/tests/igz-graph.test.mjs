...
test('header and section table parse; last section ends at EOF; second header found', () => {
  ...
  bad.writeUInt32BE(0x41424344, 0);
...
test('type table keeps empty names and reads size hints; objects enumerate with accounting == total', () => {
  ...
  assert.deepEqual(
    ...
    ['metaobject', 'tfbLightInfo', '', 'PlacementReference', 'tfbPhysicsModel'],
  );
  assert.equal(g.types[4].size_hint, 0x18 + 8 * 4);
  assert.equal(g.objects.length, 3);
  assert.deepEqual(
    g.objects.map(o => o.offset),
    objectOffsets,
  );
  ...
  assert.ok(
    g.unparsed.some(u => u.size === 0x20),
    'lead-in reported as unparsed',
  );
...
test('references resolve: strings (section-2 relative, 0 = null), objects, flagged words, floats', () => {
  ...
  assert.equal(oref.kind, 'object_ref');
  assert.equal(oref.target.type_name, 'tfbPhysicsModel');
  const flagged = plc.fields.find(f => f.offset === plc.offset + 0x18);
  assert.equal(flagged.kind, 'flagged_ref');
  assert.equal(flagged.target.low24, 4);
  assert.ok(
    phys.fields.filter(f => f.value === 0).every(f => f.kind === 'unknown'),
    'zero words are never string refs',
  );
});

test('a non-IGZ buffer is refused with exit code 2 semantics', () => {
  assert.throws(
    () => buildGraph(Buffer.from('IGA\x1a\x04\x00\x00\x00' + '\0'.repeat(64), 'latin1')),
    e => e.exitCode === 2,
  );
});
