...
function noise(n) { const b = Buffer.alloc(n); for (let i = 0; i < n; i++) b[i] = (i * 73 + 11) & 0xff; return b; }

test('planted f32 triples are found in both byte orders with their offsets and endianness', () => {
  const buf = noise(4096);
  buf.writeFloatBE(100, 512); buf.writeFloatBE(20, 516); buf.writeFloatBE(300, 520);
  buf.writeFloatLE(-12.5, 1024); buf.writeFloatLE(0, 1028); buf.writeFloatLE(7, 1032);
...
test('scalar scan honours the range and the 4x4 matrix scan requires a homogeneous last column or row', () => {
  ...
  buf.writeFloatBE(50000, 0); buf.writeFloatBE(12.25, 8);
...
test('string scan and identifier histogram surface IGZ-like type names', () => {
  const buf = Buffer.concat([Buffer.alloc(20, 1), Buffer.from('igObjectList\0igTextureAttr2\0\0igObjectList\0', 'latin1'), Buffer.alloc(9, 2)]);
...
test('bindiff reports runs with context and decodes 4-byte runs as big-endian floats', () => {
  const a = noise(300), b = Buffer.from(a);
  a.writeFloatBE(1.5, 100); b.writeFloatBE(-123.456, 100);   // all four bytes differ
  b[250] ^= 0xff; b[251] ^= 0xff;
