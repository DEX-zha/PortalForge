...
function fixture(t) {
  const built = syntheticLevel(),
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ssa-catalog-'));
...
test('confirmed drop is one reversible edit and consumes its token', t => {
  const s = fixture(t),
    before = Buffer.from(s.buffer),
    i = intent(s),
    source = structuredClone(s.placements[0]);
  const draft = prepareDrop(s, i),
    result = commitDrop(s, { token: draft.token, acknowledged: [] });
  assert.equal(result.rebuild_scene, true);
  assert.equal(s.edits.length, 1);
  assert.deepEqual(result.placement.position, i.position);
  assert.deepEqual(
    s.placements.find(p => p.offset === source.offset),
    source,
  );
  const after = Buffer.from(s.buffer);
  assert.throws(() => commitDrop(s, { token: draft.token }), /STALE_PLAN/);
  assert.equal(undo(s).rebuild_scene, true);
  assert.deepEqual(s.buffer, before);
  assert.equal(redo(s).rebuild_scene, true);
  assert.deepEqual(s.buffer, after);
});
test('stale, locked, invalid and scripted drops cannot write', t => {
  const s = fixture(t),
    i = intent(s),
    draft = prepareDrop(s, i);
...
test('catalog search combines case-insensitive words, layers and availability without merging names', () => {
  ...
  assert.deepEqual(
    filterCatalog(rows, 'plant', 'available').map(p => p.offset),
    [1],
  );
  assert.deepEqual(
    filterCatalog(rows, '', 'resource').map(p => p.offset),
    [2],
  );
...
test('catalog HTTP contract prepares without writing and commits only the server intent', async t => {
  const s = fixture(t),
    server = await startServer({ session: s, port: 0 });
  t.after(() => server.close());
  const post = (route, body) =>
    fetch(server.url + '/api/catalog/' + route, {
      method: 'POST',
  ...
  const i = intent(s),
    draft = await (await post('prepare', i)).json();
  assert.equal(s.edits.length, 0);
  s.locked = true;
  assert.equal((await post('commit', { token: draft.token })).status, 409);
  s.locked = false;
  const r = await post('commit', { token: draft.token, target: i.source, position: [0, 0, 0] });
  assert.equal(r.status, 200);
  const b = await r.json();
  assert.equal(b.placement.offset, i.target);
  assert.deepEqual(b.placement.position, i.position);
  assert.equal(b.rebuild_scene, true);
  assert.equal(s.edits.length, 1);
});
...
test('reset restores exact bytes across transforms and replacements, preserves redo and detaches old artefacts', t => {
  ...
  applyEdit(s, { kind: 'transform', target: i.source, position: [8, 9, 10] });
  const draft = prepareDrop(s, i);
  commitDrop(s, { token: draft.token });
  const changed = Buffer.from(s.buffer);
  applyEdit(s, { kind: 'transform', target: i.source, heading: 40 });
  const future = Buffer.from(s.buffer);
  undo(s);
  s.lastSave = { file: 'existing-save.decoded' };
  s.lastPatch = { dir: 'existing-patch' };
  const stale = prepareDrop(s, i);
  const result = resetScene(s);
  assert.equal(result.reset_count, 2);
  assert.equal(result.rebuild_scene, true);
  assert.deepEqual(s.buffer, original);
  assert.deepEqual(fs.readFileSync(s.file), original);
  assert.equal(s.edits.length, 0);
  assert.equal(s.undone.length, 3);
  assert.equal(s.dirty, false);
  ...
  redo(s);
  redo(s);
  assert.deepEqual(s.buffer, changed);
  redo(s);
  assert.deepEqual(s.buffer, future);
});
...
test('reset HTTP is locked during gameplay and repeated reset keeps the redo chain', async t => {
  ...
  t.after(() => server.close());
  const reset = () =>
    fetch(server.url + '/api/reset', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
  ...
  assert.equal(r.reset_scene, true);
  assert.equal(r.undo_depth, 0);
  assert.equal(r.redo_depth, 1);
  const again = await (await reset()).json();
  assert.equal(again.reset_count, 0);
  assert.equal(again.redo_depth, 1);
});
