...
function level() {
  const types = Array.from({ length: 105 }, (_, i) =>
    i === 52
      ? 'layer'
      : i === 64
        ? 'model'
  ...
  const built = buildIgz({
    types,
    strings: ['Crate_01', 'Props', 'C:/tfb/Content/Models/Objects/crate.mdl', 'Crate.ai', 'Barrel_01'],
    objects: [
      {
        type: 104,
        size: 0x100,
        fields: [
          { at: 0x08, u32: 0x01000000 },
          { at: 0x24, f32: 12.5 },
          { at: 0x28, f32: 3 },
          { at: 0x2c, f32: -7 },
          { at: 0x34, f32: 90 },
  ...
  const fixups = {
    ...
    pointer_words: [L + 0x20, L + 0x24, C + 0xdc, C + 0xe0, B + 0xa8, B + 0xdc, B + 0xe0],
  };
...
test('editor: listByLayer groups placements and classifies scripted ones as potentially unsafe', () => {
  ...
  assert.equal(res.layers.length, 1);
  assert.equal(res.layers[0].name, 'Props');
  assert.equal(res.layers[0].count, 2);
  assert.equal(res.layers[0].scripted, 1);
  assert.equal(res.layers[0].static, 1);
  const crate = getPlacement(lvl, C),
    barrel = getPlacement(lvl, B);
  assert.equal(crate.safety.safety, 'static');
  assert.equal(barrel.safety.safety, 'scripted / potentially unsafe');
  assert.match(barrel.safety.reasons[0], /behaviour script/);
  assert.equal(
    classify({ name: 'CS_PortalEntry01', layers: ['OpeningCS'], script: null, model: null }).safety,
    'marker / no model',
  );
});
...
test('editor: setTransform changes only the requested floats, refuses scripted placements without --allow-scripted', () => {
  ...
  assert.equal(r.plan.validation.status, 'VALID');
  assert.equal(r.buffer.length, lvl.buf.length);
  assert.equal(r.buffer.readFloatBE(C + 0x24), 1);
  assert.equal(r.buffer.readFloatBE(C + 0x2c), 3);
  assert.equal(r.buffer.readFloatBE(C + 0x34), 45);
  assert.equal(r.buffer.readFloatBE(C + 0xb8), 50);
  let changed = 0;
  for (let p = 0; p + 4 <= lvl.buf.length; p += 4) if (r.buffer.readUInt32BE(p) !== lvl.buf.readUInt32BE(p)) changed++;
...
test('editor: replacePlacement on unwrapped records uses the t104-generic recipe, keeps the slot name, refcount and companions, flags it as unconfirmed', () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ssa-editor-'));
  ...
  assert.match(r.plan.recipe, /t104-generic/);
  assert.ok(r.plan.validation.warnings.length);
  ...
  assert.equal(r.buffer.readFloatBE(B + 0x24), 5);
  assert.equal(r.buffer.readFloatBE(B + 0x2c), 7);
  assert.deepEqual(r.plan.kept_fields, ['+0x8', '+0xe0']);
  assert.throws(
    () => replacePlacement(lvl, B, C, { findingId: 'test.spawn', findingsOpts: { dir } }),
    /allow-scripted/,
  ); // scripted source refused
});
