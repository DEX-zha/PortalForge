...
test('cloning appends the object, shifts later sections, assigns a unique id and re-validates', () => {
  ...
  save(finding('test.guess', 'LIKELY'), { dir });
  const { buf, objectOffsets } = buildIgz({
    objects: [
      {
        type: 4,
        size: 0x40,
  ...
  const r = planClone(buf, {
    objectOffset: objectOffsets[0],
    findingId: 'test.spawn',
    edits: [{ offset: 0x20, type: 'f32be', value: 99.73 }],
    findingsOpts: { dir },
  });
  ...
  assert.throws(
    () => planClone(buf, { objectOffset: objectOffsets[0], findingId: 'test.guess', findingsOpts: { dir } }),
    /needs CONFIRMED/,
  );
});
