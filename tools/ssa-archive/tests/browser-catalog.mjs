...
const file = path.join(out, 'level.decoded');
fs.copyFileSync('.local/workspaces/tutorial-bld/entries/3-level.bld.decoded', file);
const session = openSession(file, {
  archive: 'level/Level_027_Tutorial.bld',
  entry: 3,
  fixups: JSON.parse(fs.readFileSync('.local/dolphin-evidence/ptr-scan3-fixups.json')),
});
...
try {
  ...
  assert.equal(
    ...
    directEntryConfirmed() ? 'direct-test' : 'test',
  );
  assert.equal(
    await evaluate(`document.querySelector('#launch-mode [value="direct-play"]').disabled`),
  ...
  await waitFor(
    `document.querySelectorAll('[data-object]').length === 673 && document.getElementById('skip-intro').checked`,
  );
  await evaluate(
    `window.skipRequests=[];window.originalFetch=window.fetch;window.fetch=async(url,options)=>{if(url==='/api/launch'&&options?.method==='POST'){window.skipRequests.push(JSON.parse(options.body));return new Response('{}',{headers:{'Content-Type':'application/json'}});}return window.originalFetch(url,options);};`,
  );
  for (const [mode, expected] of [
    ...
    await evaluate(
      `document.getElementById('launch-mode').value=${JSON.stringify(mode)};document.getElementById('launch-mode').dispatchEvent(new Event('change'));`,
    );
    ...
    await waitFor(
      `window.skipRequests.at(-1)?.mode===${JSON.stringify(mode)} && document.getElementById('launch-mode').disabled===false`,
    );
    assert.equal(await evaluate(`window.skipRequests.at(-1).skip_intro`), expected, 'effective option follows mode');
  }
  await evaluate(
    `document.getElementById('skip-intro').click();document.getElementById('do-launch').disabled=false;document.getElementById('do-launch').click()`,
  );
  ...
  await evaluate(
    `window.fetch=window.originalFetch;document.getElementById('launch-mode').value=${JSON.stringify(directEntryConfirmed() ? 'direct-test' : 'test')};document.getElementById('launch-mode').dispatchEvent(new Event('change'));`,
  );
  await shot('workspace-overview');
  const compatibility = await evaluate(
    `(async()=>{const b=await(await fetch('/api/catalog')).json();return {counts:b.compatibility.counts,families:b.compatibility.families.length,barrel:b.entries.find(p=>p.name==='Barrel').addition,coin:b.entries.find(p=>p.name==='1_Coper(1)').addition,chompy:b.entries.find(p=>p.name==='Enemy_ChompyNipper').addition};})()`,
  );
  assert.ok(compatibility.families > 1);
  assert.equal(compatibility.barrel.testable, true);
  assert.equal(compatibility.coin.testable, true);
  assert.equal(compatibility.chompy.testable, true);
  assert.ok(
    await evaluate(
      `document.getElementById('test-addition') && document.getElementById('test-addition-batch') && document.getElementById('stop-addition-test')`,
    ),
  );
  ...
  assert.ok(
    await evaluate(`document.getElementById('stop-launch').getBoundingClientRect().right <= innerWidth`),
    'toolbar fits 1280px',
  );
  ...
  assert.equal(
    await evaluate(
      `document.querySelectorAll('summary button, summary a, summary input, summary select, summary textarea, summary [tabindex], summary [role="button"]').length`,
    ),
    0,
    'summaries contain no nested interactive controls',
  );
  ...
  assert.equal(
    await evaluate(`document.querySelector('#folder-tree details').open`),
    !initiallyOpen,
    'Space toggles a folder',
  );
  assert.ok(
    await evaluate(`document.querySelectorAll('[data-object]').length < 673`),
    'keyboard folder selection filters the grid',
  );
  ...
  const subject = process.argv.includes('--clamper')
    ...
    : process.argv.includes('--weed')
      ? { source: 3452000, search: 'weed_2_Template(8)', label: 'weed', script: null }
      : process.argv.includes('--chompy')
        ? { source: 2390540, search: 'Enemy_ChompyNipper', label: 'chompy', script: 2348068 }
        : process.argv.includes('--coin')
          ? { source: 3034548, search: '1_Coper', label: 'coper', script: 3034796 }
          : process.argv.includes('--barrel')
            ? { source: 3983352, search: 'Barrel', label: 'barrel', script: 2739632 }
            : { source: 3446244, search: 'SUNFLOWER', label: 'sunflower', script: null };
  const search = subject.search;
  const filterMs = await evaluate(
    `(() => { const t=performance.now(), e=document.getElementById('object-search'); e.value=${JSON.stringify(search)}; e.dispatchEvent(new Event('input')); return performance.now()-t; })()`,
  );
  ...
  assert.equal(
    await evaluate(`document.querySelector('[data-hierarchy="${source}"]').classList.contains('selected')`),
    true,
  );
  ...
  const drop = async ({ part = '.asset-preview', x = 0.65, y = 0.6, cancel = false, outside = false } = {}) => {
    const coords = await evaluate(
      `(() => {const el=document.querySelector('[data-object="${source}"] ${part}');el.scrollIntoView({block:'nearest'});const b=el.getBoundingClientRect(),r=document.getElementById('c').getBoundingClientRect();return {x:b.left+b.width/2,y:b.top+b.height/2,dx:${outside ? '50' : `r.left+r.width*${x}`},dy:${outside ? '120' : `r.top+r.height*${y}`}};})()`,
    );
    ...
    for (let i = 1; i <= 12; i++) {
      await send('Input.dispatchMouseEvent', {
        ...
        button: 'left',
        buttons: 1,
        x: coords.x + ((coords.dx - coords.x) * i) / 12,
        y: coords.y + ((coords.dy - coords.y) * i) / 12,
      });
      await sleep(20);
    }
  ...
  assert.equal(session.edits.length, 1);
  assert.equal(session.placements.length, 674);
  assert.deepEqual(session.buffer, before);
  assert.equal(await evaluate("document.getElementById('drop-dialog').open"), false);
  assert.ok(session.additions[0].position.every(Number.isFinite));
  await shot('first-addition');
  const secondHint = await drop({ part: '.object-name', x: 0.45, y: 0.7 });
  await waitFor(
    "document.querySelector('[data-hierarchy=\"-2\"]') && document.getElementById('dirty').textContent.startsWith('2 ')",
  );
  assert.equal(session.placements.length, 675);
  assert.equal(session.additions.length, 2);
  assert.deepEqual(session.buffer, before);
  ...
  await evaluate(
    "const f=document.querySelector('[data-edit=\"heading\"]');f.value=240;f.dispatchEvent(new Event('change',{bubbles:true}));",
  );
  ...
  for (let i = 2; i >= 0; i--) {
    ...
    await waitFor(
      i === 0
        ? "document.getElementById('undo').disabled && !document.getElementById('redo').disabled"
        : "!document.getElementById('undo').disabled && document.getElementById('dirty').textContent.startsWith('" +
            i +
            " ')",
    );
  }
  assert.equal(session.additions.length, 0);
  assert.equal(session.placements.length, 673);
  assert.deepEqual(session.buffer, before);
  for (let i = 1; i <= 3; i++) {
  ...
  const savedFile = session.lastSave.file,
    savedBytes = fs.readFileSync(savedFile),
    savedSidecar = fs.readFileSync(savedFile + '.portalforge.json');
  ...
  assert.equal(session.additions.length, 0);
  assert.equal(session.undone.length, 3);
  assert.deepEqual(session.buffer, before);
  assert.equal(session.lastSave, null);
  assert.equal(session.lastPatch, null);
  assert.deepEqual(fs.readFileSync(savedFile), savedBytes);
  assert.deepEqual(fs.readFileSync(savedFile + '.portalforge.json'), savedSidecar);
  assert.equal(
    await evaluate("document.getElementById('do-launch').disabled && document.getElementById('do-patch').disabled"),
    true,
  );
  ...
  const result = {
    ...
    checks: [
      '673 original objects preserved',
      'two mouse drops at distinct destinations',
      'no victim',
      'negative identity selection',
      'rotation',
      'scale disabled',
