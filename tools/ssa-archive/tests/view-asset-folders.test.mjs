...
test('folder navigation partitions every entry once without merging instances or changing records', () => {
  ...
  const original=structuredClone(entries), tree=folderTree(entries);
  assert.equal(tree.reduce((n,r)=>n+r.count,0),4);
  for(const root of tree)assert.equal(root.count,root.children.reduce((n,c)=>n+c.count,0));
  assert.deepEqual(entries.filter(e=>inFolder(e,['Vegetation','Flowers'])).map(e=>e.offset),[1,2]);
