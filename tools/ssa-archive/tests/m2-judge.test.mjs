...
test('judge --replicates counts runs of another experiment with the byte-identical rebuilt archive toward SC-004, and refuses a different input or an unjudged replica', () => {
  ...
  const other = record('c', [run()]); other.inputs.rebuilt_sha256 = 'z'; fs.writeFileSync(path.join(dir, 'c.json'), JSON.stringify(other));
  assert.throws(() => judge({ id: 'a', run: 1, observed: 'moved', match: true, replicates: ['b'], dir }), /not a fully judged/);   // b not judged yet
  ...
  assert.equal(r.status, 'PASS'); assert.equal(r.replicates[0].experiment_id, 'b'); assert.match(r.notes, /2\/2 runs of the same input/);
  assert.throws(() => judge({ id: 'a', run: 1, observed: 'moved', match: true, replicates: ['c'], dir }), /not the same input/);
  assert.throws(() => judge({ id: 'a', run: 1, observed: 'moved', match: true, replicates: ['a'], dir }), /cannot replicate itself/);
});
