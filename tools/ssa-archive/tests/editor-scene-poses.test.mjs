...
test('bridge and cannon previews match the tutorial actor poses read in Dolphin', { skip }, () => {
  ...
  const before = Buffer.from(s.buffer), previews = scriptedPreviews(s);
  const dock = previews.find(p => p.owner === 1776460);
  const bridge = previews.find(p => p.owner === 1875316);
  assert.equal(dock.heading, -46); assert.equal(dock.template, 1783120);
  assert.equal(bridge.heading, -44); assert.equal(bridge.template, 1777576);
  const cannon = previews.filter(p => p.owner === 2302624);
  assert.deepEqual(cannon.map(p => [p.template, p.heading]), [[2155316, 156], [2169700, 0]]);
  ...
  assert.deepEqual(roles.find(r => r.offset === 4124024).counterparts, [{offset:1823456,name:'Legendary_Treasure_Ancient_Shell(1)'}]);
