...
import {
  ...
  resolveAll,
  resolveModel,
  validateAgainstFixups,
  FIELDS,
} from '../src/igz/model-resolve.mjs';
...
function level({ placementType = 77, modelType = 12 } = {}) {
  ...
  const strings = [
    'Crate_01',
    'C:/tfb/Content/Models/Objects/crate.mdl',
    'Barrel_01',
    'CS_Camera01',
    'C:/tfb/Content/Levels/L/Scripts/PushBlock_Template.ai',
    'Block_01',
    'C:/tfb/Content/Models/Objects/barrel.mdl',
  ];
  const at = [];
  let acc = 0;
...
test('model-resolve finds an indirect model and flags an ambiguous one', () => {
  ...
  const ctx2 = levelContext(buf, buildGraph(buf, { fields: false }), null);
  detectClasses(ctx2);
...
test('model-resolve agrees with a runtime fixup map when one is available', () => {
  ...
  const fixups = {
    ...
    pointer_words: [crate + FIELDS.model, barrel + FIELDS.model, block + FIELDS.model, block + FIELDS.behavior],
  };
...
test('safety rules: track-bound behaviour is critical, a plain script is medium, a shared model rewrite is critical, a span mismatch blocks', () => {
  ...
  const scripted = assessPlacement(
    { ...block, behavior: { path: 'C:/x/Scripts/027_WindmillProp.ai' } },
    { hasRuntimeMap: true },
  );
  ...
  const shared = assessReplacement(crate, block, {
    hasRuntimeMap: true,
    sharedInVictim: [{ offset: 0x100, users: 26, path: 'C:/tfb/Content/Models/Objects/plants_weed2_whole.mdl' }],
  });
  ...
  const contained = assessReplacement(crate, marker, {
    hasRuntimeMap: true,
    sharedInVictim: [{ offset: 0x100, users: 1, path: 'crate.mdl' }],
  });
...
test('placement v1 is frozen: every produced record validates against placement-v1.schema.json', async () => {
  ...
  for (const r of res.rows)
    assert.ok(validate(r), `row 0x${r.offset.toString(16)}: ${JSON.stringify(validate.errors)}`);
...
test('layers: a named list record that references a placement is a layer; scripts, models and other placements are not', async () => {
  const {
    ...
    placementReferrers,
    layersOf,
  } = await import('../src/igz/model-resolve.mjs');
  const strings = [
    'Crate_01',
    'C:/tfb/Content/Models/Objects/crate.mdl',
    'Plants',
    'C:/tfb/Content/Levels/L/Scripts/Hat_Pickup.ai',
    'Level_027_plants.lvl',
  ];
  const at = [];
  let acc = 0;
  for (const s of strings) {
  ...
  const built = buildIgz({
    ...
    objects: [
      ...
      }, // "Plants" list -> the first placement
      {
        type: 90,
        size: 0x40,
        fields: [
      ...
      }, // a .ai script also referencing it
      {
        type: 41,
        size: 0x40,
        fields: [
      ...
      }, // "Level_027_plants.lvl" registry
    ],
    headTable: [0, 1, 2, 3, 4, 5, 6],
  });
  const g = buildGraph(built.buf, { fields: false });
  const ctx = lc(built.buf, g, null);
  dc(ctx);
...
test('corpusReport aggregates rates, schema conformance and safety over several files', async () => {
  ...
  const a = path.join(dir, 'a.igz'),
    b = path.join(dir, 'b.igz');
