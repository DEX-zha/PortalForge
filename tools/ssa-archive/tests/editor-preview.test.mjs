...
test('the render puts ink on the page, which is what "the canvas shows nothing" was about', () => {
  ...
  assert.ok(inkFraction(r.image) > 0.001, `only ${(inkFraction(r.image) * 100).toFixed(3)}% of the picture is not background`);
});

test('a proxy is big enough to see and to click on a real level', { skip: !haveSamples && 'local samples absent' }, () => {
  for (const file of [TUTORIAL, MINING]) {
    const r = renderPreview(open(file), { out: null, width: 1100, height: 780 });
    assert.ok(r.proxy_px_median >= LEGIBLE_PX,
      `${path.basename(file)}: ${r.proxy_px_median}px median is below the ${LEGIBLE_PX}px floor`);
    assert.ok(r.on_screen / r.drawn >= 0.95,
...
test('the fixed 1.6-unit proxy that caused the bug would fail this suite', { skip: !haveSamples && 'local samples absent' }, () => {
  ...
  assert.ok(oldPx < LEGIBLE_PX, `the old fixed size would have been ${oldPx.toFixed(1)}px, which should be illegible`);
  assert.ok(r.proxy_px_median > oldPx * 2, 'the derived size is not meaningfully bigger than the fixed one');
});

test('every level gets a proxy of its own size, so nothing is tuned to the tutorial', { skip: !haveSamples && 'local samples absent' }, () => {
  ...
  assert.ok(Math.abs(a.proxy_px_median - b.proxy_px_median) < a.proxy_px_median,
