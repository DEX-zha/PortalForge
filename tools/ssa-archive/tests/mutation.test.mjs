...
test('a same-size one-value mutation of an uncompressed entry changes exactly one CONTENT region', () => {
  ...
  const repl = path.join(dir, 'mut.bin'); fs.writeFileSync(repl, data);
...
test('mutating a compressed entry re-encodes only that entry: strategy REENCODE_REPLACED, other entries byte-identical', () => {
  ...
  const entries = [{ name: 'level.bld', data: stored, size: text.length, mode: 0x10000000 }, { name: 'ENGLISH.pak', data: Buffer.alloc(300, 7) }];
  ...
  const mutated = Buffer.from(text); mutated.write('position 200', mutated.indexOf('position 100'), 'latin1');
  const repl = path.join(dir, 'mut.bin'); fs.writeFileSync(repl, mutated);
  ...
  const back = decodeStoredEntry(r.buffer.subarray(e.start, e.start + e.stored_size), e, { values: p.chunk_area.values, word_24: p.header.word_24, word_28: p.header.word_28 });
