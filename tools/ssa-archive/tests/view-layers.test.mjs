...
const level = [
  P(0x10, ['Plants', 'Level_027_plants.lvl']),
  P(0x20, ['Plants']),
  P(0x30, ['Loot']),
  P(0x40, []),
];
...
test('layers: a placement is visible while at least one of its layers is visible', () => {
  ...
  assert.deepEqual([...visibleSet(level, state)].sort((a, b) => a - b), [0x10, 0x20, 0x30, 0x40]);
...
test('layers: the index counts each placement once per layer it claims and is ordered by size', () => {
  const idx = layerIndex(level);
  assert.deepEqual(idx.map(l => [l.name, l.count]), [['Plants', 2], ['(unlayered)', 1], ['Level_027_plants.lvl', 1], ['Loot', 1]]);
  assert.equal(idx.reduce((n, l) => n + l.count, 0), 5, 'the first placement is counted in both of its layers');
});
