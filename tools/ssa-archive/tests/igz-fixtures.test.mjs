...
const SKIP = {
  skip: fs.existsSync(level)
    ? false
    : 'decoded tutorial level.bld not present (run: node cli.mjs extract <Level_027_Tutorial.bld> --out .local/workspaces/tutorial-bld --decode)',
};

test(
  'tutorial level.bld: 9 sections to EOF, 224 types, accounting == total, spawn record typed with its position',
  SKIP,
  () => {
