...
test(
  'a level the editor was never built against opens with no per-level configuration',
...
test(
  'the two levels resolve different classes, so nothing is carried over from one to the other',
  { skip: !haveSamples && 'local samples absent' },
  () => {
...
test(
  'without a runtime map every pointer-derived attribute says so, and none is dressed up',
  { skip: !haveSamples && 'local samples absent' },
  () => {
...
test(
  'a transform still works without a runtime map, because reading the layout does not need one',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    const s = openSession(MINING, { archive: 'a', entry: 3, deps: gates });
    ...
    const a = fs.readFileSync(MINING),
      b = fs.readFileSync(out);
    let differing = 0;
...
test(
  'duplication without a runtime map is refused, because rewriting pointers needs evidence reading does not',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    const s = openSession(MINING, { archive: 'a', entry: 3, deps: gates });
    const a = s.placements.find(p =>
    ...
    assert.throws(
      () => applyEdit(s, { kind: 'replace', target: b.offset, source: a.offset }),
...
test(
  'the runtime map changes what the editor CLAIMS, never what it writes',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    ...
    const map = JSON.parse(
      fs.readFileSync(path.resolve(here, '../../../.local/dolphin-evidence/ptr-scan3-fixups.json'), 'utf8'),
    );
    const edit = fixups => {
      ...
      applyEdit(s, {
        kind: 'transform',
        target: p.offset,
        position: [p.position[0], 16, p.position[2]],
        heading: 200,
        scale: 150,
      });
    ...
    const withMap = edit(map),
      without = edit(null);

    assert.ok(
      fs.readFileSync(withMap.written).equals(fs.readFileSync(without.written)),
    ...
    const a = withMap.s.placements.find(p => p.model.path),
      b = without.s.placements.find(p => p.offset === a.offset);
