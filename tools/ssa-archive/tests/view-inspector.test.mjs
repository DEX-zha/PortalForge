...
test(
  'inspector: a scripted prop shows its model, its script, its layers and the warning that goes with it',
...
test(
  'inspector: a shared model record is spelled out with its user count before any duplication is offered',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    const { at } = tutorial();
    const weed = at(0x34ac60); // weed_2_Template(8), model shared by 26
    const html = renderPlacement(weed, assessPlacement(weed, { hasRuntimeMap: true }), [
      { offset: 0x3495e4, name: 'sunflower_Template(1)', span: weed.span },
    ]);
...
test(
  'inspector: an attribute with no runtime evidence is not dressed up as one that has it',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    const buf = fs.readFileSync(TUTORIAL);
...
test(
  'inspector: the layer grade bar is proportional and names each severity',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    const { res } = tutorial();
    ...
    assert.ok(
      ...
      'the bar accounts for every placement of the layer',
    );
...
test(
  'inspector: on a level with no runtime map, duplication says why it is unavailable',
  { skip: !haveSamples && 'local samples absent' },
  () => {
    const { at } = tutorial();
    const p = at(0x3495e4);
