...
const text = n => { const words = ['igObjectList', 'tfbPhysicsModel', 'PlacementReference', 'ScriptSet', 'CameraInfo']; let s = ''; while (s.length < n) s += words[s.length % words.length] + ' '; return Buffer.from(s.slice(0, n), 'latin1'); };
...
test('growing a multi-chunk entry by one chunk keeps the archive VALID, decodable and shifts the following table indexes', () => {
  ...
  const dataA = text(0x8000 + 3000), dataB = text(0x8000 + 100);
  const encA = encodeEntryChunks(dataA), encB = encodeEntryChunks(dataB);
  assert.equal(encA.chunk_count, 2); assert.equal(encB.chunk_count, 2);
  const entries = [
  ...
  const a = m.entries.find(e => e.name === 'level.bld'), b = m.entries.find(e => e.name === 'other.bld');
  const grown = text(2 * 0x8000 + 500);                    // needs 3 chunks
  const repl = path.join(dir, 'grown.bin'); fs.writeFileSync(repl, grown);
  ...
  const pa = p.entries[a.index], pb = p.entries[b.index];
  ...
  assert.equal(Buffer.compare(decodeStoredEntry(r.buffer.subarray(pa.start, pa.start + pa.stored_size), pa, tables), grown), 0);
  assert.equal(Buffer.compare(decodeStoredEntry(r.buffer.subarray(pb.start, pb.start + pb.stored_size), pb, tables), dataB), 0);
