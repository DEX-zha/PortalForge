...
test('clone anchor accepts only a bounded singleton me expression, never a variable or a truncated list', () => {
  const buffer = Buffer.alloc(256), sec = {offset: 32, size: 224};
  const session = {buffer, graph:{sections:[sec], object_section:0}};
  const op = 40, list = 120, array = 152;
  buffer.writeUInt32BE(list-sec.offset, op+0x28);
  buffer.writeUInt32BE(114,list);buffer.writeUInt32BE(1,list+8);buffer.writeUInt32BE(1,list+12);
  buffer.writeUInt32BE(0x80000004,list+16);buffer.writeUInt32BE(array-sec.offset,list+20);
  buffer.writeUInt32BE(0x8000005b,array);
  assert.equal(cloneAtOwner(session,op),true);
  buffer.writeUInt32BE(0x8000000a,array);assert.equal(cloneAtOwner(session,op),false);
  buffer.writeUInt32BE(0x8000005b,array);buffer.writeUInt32BE(2,list+8);assert.equal(cloneAtOwner(session,op),false);
  buffer.writeUInt32BE(9999-sec.offset,op+0x28);assert.equal(cloneAtOwner(session,op),false);
...
test('Barrel resource links to active counterparts; a shared model does not share their positions', {skip}, () => {
  const s=tutorial(), template=at(s,3983352), barrel=at(s,3074164);
  ...
  const before=Buffer.from(s.buffer), templateBefore={...template,position:[...template.position]};
  ...
  const changes=[];for(let i=0;i<before.length;i+=4)if(before.readUInt32BE(i)!==moved.readUInt32BE(i))changes.push(i);
  assert.deepEqual(changes,[0x24,0x28,0x2c].map(n=>n+barrel.offset));
  assert.equal(scriptDiagnostics(s,barrel).movement.kind,'initial_position');
  assert.deepEqual(scriptDiagnostics(s,template).movement.targets.find(p=>p.offset===barrel.offset).position,[88,11.5,40],'links use current positions, not cached startup coordinates');
  undo(s);assert.deepEqual(s.buffer,before);redo(s);assert.deepEqual(s.buffer,moved);
});

test('a cloned bridge resource offers conditional creator anchors instead of its storage position', {skip}, () => {
  const s=tutorial(), template=at(s,1777576);
  const d=scriptDiagnostics(s,template);
  const link=d.movement.targets.find(p=>p.offset===1875316);
  assert.equal(link.relation,'creator');assert.equal(link.conditional,true);
