...
test('direct test restores before transition, proves current consumption and restores the slot after stopping',async()=>{
 const f=fixture();const entryServices={confirmed:()=>true,prepare:async()=>{f.calls.push(['prepare']);return {file:'checkpoint'};},restore:async()=>{f.calls.push(['restore-entry']);return {proof:{state_sha256:'hash'},restore:()=>f.calls.push(['restore-slot'])};}};
 const r=await runEditorGame({...f,mode:'direct-test',archive:'level/Level_027_Tutorial.bld',gameFactory:()=>f.fake,evidenceDir:f.dir,entryServices});
 assert.deepEqual(f.calls.map(c=>c[0]),['prepare','launch','restore-entry','macro','stop','close','restore-slot']);
 assert.equal(r.consumption.verified,true);assert.equal(r.entry.state_sha256,'hash');assert.equal(r.entry_slot_restored,true);
 const steps=f.calls.find(c=>c[0]==='macro')[1];assert.equal(steps[0].press,'A');assert.ok(!steps.some(s=>s.shot==='title'));
});
test('direct entry refuses an unvalidated level or recipe before launch',async()=>{
 const f=fixture();const args={...f,mode:'direct-test',gameFactory:()=>f.fake,evidenceDir:f.dir,entryServices:{confirmed:()=>false}};
 await assert.rejects(runEditorGame({...args,archive:'level/Level_000_Mining.bld'}),/not yet validated/);
 await assert.rejects(runEditorGame({...args,archive:'level/Level_027_Tutorial.bld'}),/still being validated/);assert.deepEqual(f.calls,[]);
});
test('direct play reaches the tutorial, leaves movement to the player and closes cleanly',async()=>{
 const f=fixture(),entryServices={confirmed:()=>true,prepare:async()=>({}),restore:async()=>({proof:{},restore:()=>f.calls.push(['restore-slot'])})};
 const r=await runEditorGame({...f,mode:'direct-play',archive:'level/Level_027_Tutorial.bld',gameFactory:()=>f.fake,evidenceDir:f.dir,entryServices,pollMs:0});
 const steps=f.calls.find(c=>c[0]==='macro')[1];assert.equal(steps.at(-1).shot,'tutorial-skylander');assert.ok(!steps.some(s=>s.nunchuk));
 assert.equal(r.status,'CLOSED');assert.equal(r.entry_slot_restored,true);assert.equal(r.consumption.verified,true);
});
test('skip selection reaches every automated launch mode and direct play hands off before movement',async()=>{
 for(const mode of ['test','direct-test','direct-play']){
  const f=fixture(),entryServices={confirmed:()=>true,prepare:async()=>({}),restore:async()=>({proof:{},restore:()=>{}})};
  const r=await runEditorGame({...f,mode,skip_intro:true,archive:'level/Level_027_Tutorial.bld',gameFactory:()=>f.fake,evidenceDir:f.dir,entryServices,pollMs:0});
  const steps=f.calls.find(c=>c[0]==='macro')[1];assert.equal(r.skip_intro,true);assert.equal(steps.filter(s=>s.nunchuk?.C).length,1);assert.equal(r.consumption.verified,true);
...
test('a failed entry restore preserves its cleanup until the owned process stops',async()=>{
 const f=fixture(),entryServices={confirmed:()=>true,prepare:async()=>({}),restore:async()=>{throw Object.assign(Error('incompatible state'),{restoreEntry:()=>f.calls.push(['restore-slot'])});}};
 await assert.rejects(runEditorGame({...f,mode:'direct-test',archive:'level/Level_027_Tutorial.bld',gameFactory:()=>f.fake,evidenceDir:f.dir,entryServices}),/incompatible state/);
 assert.deepEqual(f.calls.map(c=>c[0]),['launch','stop','close','restore-slot']);
});
