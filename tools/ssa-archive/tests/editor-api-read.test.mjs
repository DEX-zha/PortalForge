...
test('GET /api/placements returns records that validate against the frozen contract, with their layers', async () => {
  ...
  try {
    ...
    for (const p of b.placements)
      assert.ok(validate(p), `0x${p.offset.toString(16)}: ${JSON.stringify(validate.errors)}`);
    for (const p of b.placements) {
    ...
    assert.ok(
      Object.values(props.grades).some(n => n > 0),
      'every placement of a layer is graded',
    );
  } finally {
    await s.close();
  }
...
test('GET /api/placement/:offset adds the safety rules and the duplication targets', async () => {
  ...
  try {
    ...
    assert.ok(
      b.safety.every(r => r.id && r.severity && r.message),
      'a rule states what it is and how bad it is',
    );
    assert.ok(
      b.safety.some(r => r.id === 'NO_RUNTIME_MAP'),
      'a level without a runtime map says so on every object',
    );
    assert.ok(Array.isArray(b.replace_targets));
    assert.ok(
      b.replace_targets.every(t => t.span === first.span),
      'only same-size slots are offered',
    );
    assert.ok(
      b.replace_targets.every(t => t.offset !== first.offset),
      'and never the source itself',
    );
    const hex = await (await fetch(`${s.url}/api/placement/0x${first.offset.toString(16)}`)).json();
    assert.equal(
      hex.placement.offset,
      first.offset,
      'an offset may be given in hexadecimal, as every other tool prints it',
    );
  } finally {
    await s.close();
  }
