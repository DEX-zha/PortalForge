...
const options = {
  ...
  layout: { type: 'string' },
  pad: { type: 'string' },
  'reencode-all': { type: 'boolean', default: false },
  ...
  match: { type: 'string' },
  replicates: { type: 'string', multiple: true },
  pattern: { type: 'string', multiple: true },
  poke: { type: 'string', multiple: true },
  'save-slot': { type: 'string' },
  histogram: { type: 'boolean', default: false },
  tol: { type: 'string' },
  'no-dimensions': { type: 'boolean', default: false },
  dimensions: { type: 'boolean', default: false },
  base: { type: 'string' },
  address: { type: 'string' },
  set: { type: 'string', multiple: true },
  plan: { type: 'string' },
  'append-to-list': { type: 'boolean', default: false },
  file: { type: 'string' },
  depth: { type: 'string' },
  end: { type: 'string' },
  fixups: { type: 'string', multiple: true },
  'no-register': { type: 'boolean', default: false },
  probe: { type: 'string', multiple: true },
  'also-finding': { type: 'string', multiple: true },
  'fresh-ids': { type: 'boolean', default: false },
  regions: { type: 'string' },
  'section-address': { type: 'string', multiple: true },
  'replace-entry': { type: 'string' },
  'dump-section': { type: 'boolean', default: false },
  overwrite: { type: 'string' },
  'link-after': { type: 'string' },
  'link-field': { type: 'string' },
  'replace-node': { type: 'string' },
  'replace-record': { type: 'string' },
  keep: { type: 'string' },
  near: { type: 'string' },
  layer: { type: 'string' },
  all: { type: 'boolean' },
  ...
  'no-refcounts': { type: 'boolean', default: false },
  'dump-anchor': { type: 'string' },
  width: { type: 'string' },
  height: { type: 'string' },
...
const readArchive = file => {
  ...
  if (!fs.existsSync(p)) throw new CliError('File not found: ' + p, 3);
...
const commands = {
  ...
  async list(pos) {
    ...
    const rows = parsed.entries.map(e => ({
      index: e.index,
      name: e.name,
      hash: '0x' + (e.hash >>> 0).toString(16).padStart(8, '0'),
      start: e.start,
      size: e.size,
      stored_size: e.stored_size,
      mode: '0x' + (e.mode >>> 0).toString(16).padStart(8, '0'),
      compression: e.compression,
      chunk_table_index: e.chunk_table_index,
    }));
    return {
      result: { count: rows.length, entries: rows },
      text: rows
        .map(
          r =>
  ...
  async extract(pos, o) {
    ...
    let decoded = null;
    if (o.decode) {
      const { decodeWorkspace } = await import('./src/iga/decode.mjs');
      decoded = await decodeWorkspace(outDir);
    }
    return {
      result: { workspace: outDir, entries: manifest.entries.length, decoded },
      text:
        `Extracted ${manifest.entries.length} entries to ${outDir}` +
        (decoded
