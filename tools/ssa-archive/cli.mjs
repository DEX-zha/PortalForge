...
const options = {
  ...
  end: { type: 'string' }, fixups: { type: 'string' }, 'no-register': { type: 'boolean', default: false }, probe: { type: 'string', multiple: true }, 'also-finding': { type: 'string', multiple: true }, 'fresh-ids': { type: 'boolean', default: false }, regions: { type: 'string' }, 'section-address': { type: 'string', multiple: true }, 'replace-entry': { type: 'string' },
};
