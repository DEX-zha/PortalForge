...
export function lzmaDecode(stream, props, size) {
  ...
  Buffer.from(props).copy(header, 0);
...
export function lzmaEncodeChunk(data, mode = 5) {
  ...
  if (out[0] !== LZMA_PROPS[0])
    throw new Error('Encoder produced unexpected LZMA properties ' + out.subarray(0, 5).toString('hex'));
...
export function entryChunkPlan(entry, tables) {
  ...
  if (n > 1) {
    ...
    if (values.length !== n + 1)
      throw new Error(`Chunk table for entry ${entry.index} needs ${n + 1} values at index ${idx}`);
    return {
      kind: 'multi',
      chunks: values
        .slice(0, n)
        .map((v, i) => ({ offset: (v & 0x7fff) * ALIGN, compressed: (v & 0x8000) !== 0, index: i })),
      end: (values[n] & 0x7fff) * ALIGN,
      values,
    };
  }
  ...
  return {
    kind: 'single',
    chunks: [{ offset: 0, compressed, index: 0 }],
    end: units === null ? null : units * ALIGN,
    record_u16_index: rec === undefined ? null : recordU16,
  };
}
...
export function encodeEntryChunks(data, { mode = 5, allowRaw = true } = {}) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let stream = null;
    try {
      stream = lzmaEncodeChunk(slice, mode);
    } catch (e) {
      if (!allowRaw) throw e;
    }
    ...
    if (useRaw) chunk = Buffer.from(slice);
      ...
      const h = Buffer.alloc(CHUNK_HEADER);
      h.writeUInt16BE(stream.length, 0);
      LZMA_PROPS.copy(h, 2);
      chunk = Buffer.concat([h, stream]);
    }
    const padded = Buffer.alloc(alignUp(Math.max(chunk.length, 1)));
    chunk.copy(padded, 0);
    pieces.push(padded);
...
export async function decodeWorkspace(dir) {
  ...
  for (const e of compressed) {
    ...
    try {
      ...
      details.push({
        ...
        compressed_chunks: plan.chunks.filter(c => c.compressed).length,
        magic: out.subarray(0, 4).toString('latin1'),
      });
    } catch (err) {
  ...
  return {
    ...
    reason:
      decoded === compressed.length ? null : 'some entries failed to decode; see manifest chunk_tables[0].decoded',
    details,
  };
}
...
export function reencodeEntry(entry, data, manifest) {
  ...
  const original = chunkCount(entry.size),
    now = chunkCount(data.length);
  const plan = entryChunkPlan(entry, tables);
  if (now !== original && plan.kind !== 'multi')
    throw new Error(
      `REENCODE_CHUNK_COUNT_CHANGED: entry ${entry.index} is single-chunk and the replacement needs ${now} chunk(s)`,
    );
  ...
  if (plan.kind === 'multi') {
    ...
    if (delta !== 0) {
      ...
      for (const e of manifest.entries) {
        ...
        if (e.chunk_table_index > entry.chunk_table_index) {
          e.chunk_table_index += delta;
          e.mode = (e.mode & 0xff000000) | (e.chunk_table_index & 0xffffff);
        }
      }
    }
  } else if (plan.record_u16_index !== null) {
    const units = enc.stored_units,
      flags = enc.values[0] & 0x8000 ? 0x80 : 0x00;
