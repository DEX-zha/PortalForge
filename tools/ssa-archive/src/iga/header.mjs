...
export const WORD_NAMES = ['magic', 'version', 'table_size', 'count', 'flags_packed', 'word_14',
  'name_table_offset', 'name_table_size', 'word_20', 'word_24', 'word_28', 'word_2c'];
...
export function decodeHashes(buf, count, issues = []) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i > 0 && !(hashes[i - 1] < v)) {
      issues.push(failure(HEADER_SIZE + HASH_SIZE * i, `hashes[${i}]`, hex(v), `greater than ${hex(hashes[i - 1])}`, 'HASHES_NOT_SORTED'));
    }
    hashes.push(v);
  }
  return hashes;
...
export function maxProbeDistance(hashes, scale = lookupScale(hashes.length)) {
  let max = 0;
  hashes.forEach((h, i) => { const est = Math.min(hashes.length - 1, Math.floor((h >>> 0) / scale)); max = Math.max(max, Math.abs(i - est)); });
  return max;
}
