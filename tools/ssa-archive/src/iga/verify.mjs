...
export function reportFrom(failures) {
  ...
  if (failures.some(f => UNSUPPORTED.has(f.reason)) && status === 'INVALID' && failures.every(f => UNSUPPORTED.has(f.reason) || f.reason === 'BAD_MAGIC')) status = 'UNSUPPORTED';
...
export function verifyWorkspace(dir) {
  ...
  catch (e) { return { dir, ...reportFrom([{ offset: 0, field: 'manifest.json', actual: e.message, expected: 'readable JSON manifest', reason: 'WORKSPACE_ENTRY_HASH_MISMATCH' }]) }; }
  const schema = validateManifest(manifest);
  if (!schema.valid) failures.push(...schema.errors.map(err => ({ offset: 0, field: 'manifest' + err.instancePath, actual: JSON.stringify(err.params), expected: err.message, reason: 'WORKSPACE_ENTRY_HASH_MISMATCH' })));
  for (const e of manifest.entries ?? []) {
    const file = path.join(dir, e.file);
    if (!fs.existsSync(file)) { failures.push({ offset: e.start, field: `entries[${e.index}].file`, actual: 'missing', expected: e.file, reason: 'WORKSPACE_ENTRY_HASH_MISMATCH' }); continue; }
    const actual = createHash('sha256').update(fs.readFileSync(file)).digest('hex');
    if (!e.replaced && actual !== e.data_sha256) failures.push({ offset: e.start, field: `entries[${e.index}].data_sha256`, actual, expected: e.data_sha256, reason: 'WORKSPACE_ENTRY_HASH_MISMATCH' });
  }
