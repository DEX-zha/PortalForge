...
export function decode(file) {
  ...
  for (let y = 0; y < h; y++) {
    ...
    const cur = out.subarray(y * stride, y * stride + stride),
      prev = y ? out.subarray((y - 1) * stride, y * stride) : null;
    for (let x = 0; x < stride; x++) {
      const a = x >= ch ? cur[x - ch] : 0,
        b = prev ? prev[x] : 0,
        c = prev && x >= ch ? prev[x - ch] : 0;
      const v = line[x];
      let r;
      if (ft === 0) r = v;
      else if (ft === 1) r = v + a;
...
export function encode(img, file) {
  ...
  const chunk = (type, body) => {
    ...
    body.copy(b, 8);
    b.writeInt32BE(crc(Buffer.concat([Buffer.from(type, 'latin1'), body])), 8 + body.length);
    return b;
  };
...
export function diffBoxes(a, b, thr = 40, minPix = 12) {
  ...
  for (let i = 0, n = a.w * a.h; i < n; i++) {
    const pa = i * a.ch,
      pb = i * b.ch;
    if (
      Math.abs(a.data[pa] - b.data[pb]) > thr ||
      Math.abs(a.data[pa + 1] - b.data[pb + 1]) > thr ||
      Math.abs(a.data[pa + 2] - b.data[pb + 2]) > thr
    )
      mask[i] = 1;
  }
