...
export function validate(record) {
  ...
  if (record.category !== undefined) { try { assertCategory(record.category); } catch (e) { errors.push(e.message); } }
...
export function promote(id, { to, evidence, opts = {} }) {
  ...
  if (to === 'LIKELY') {
    if (independent.size < 2) throw new Error(`UNKNOWN -> LIKELY needs two independent concordant observations (have ${independent.size})`);
...
export function contradict(id, { evidence, opts = {} }) {
  ...
  r.confidence = 'UNKNOWN'; r.editable = false; r.updated = today();
  return save(r, opts);
}
...
export function render({ dir = recordsDir, outDir = findingsDir } = {}) {
  ...
  for (const [category, title] of Object.entries(CATEGORIES)) {
    ...
    const lines = [`# Findings — ${title}`, '', `Generated by \`ssa-archive findings render\` from \`docs/findings/records/\`. Do not edit by hand. Labels: CONFIRMED (experimentally demonstrated), LIKELY (concordant observations), UNKNOWN (hypothesis). Only CONFIRMED findings may be editable.`, '',
    ...
    for (const r of rows) {
      ...
      const ev = r.evidence.map(e => (e.experiment_id ? `[${e.experiment_id}] ` : e.probe ? `(${e.probe}) ` : '') + e.summary).join('<br>');
      lines.push(`| ${r.id} | ${r.structure} | ${loc} | ${r.type} | ${r.endian} | ${r.meaning.replace(/\|/g, '\\|')} | ${ev.replace(/\|/g, '\\|')} | **${r.confidence}** | ${r.editable ? 'yes' : 'no'} | ${r.updated} |`);
    }
