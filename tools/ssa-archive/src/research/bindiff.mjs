...
export function bindiff(a, b, { context = 16, limit = 1000, hexLimit = 64 } = {}) {
  ...
  while (i < min) {
    ...
    let j = i; while (j < min && a[j] !== b[j]) j++;
    runs.push({ offset: i, length: j - i }); i = j;
  }
  ...
  const rows = runs.slice(0, limit).map(r => {
    const s = Math.max(0, r.offset - context), e = Math.min(Math.max(a.length, b.length), r.offset + r.length + context);
    return { ...r, old_hex: a.subarray(r.offset, Math.min(r.offset + r.length, r.offset + hexLimit, a.length)).toString('hex'),
      new_hex: b.subarray(r.offset, Math.min(r.offset + r.length, r.offset + hexLimit, b.length)).toString('hex'),
      context_before: a.subarray(s, r.offset).toString('hex'), context_after: a.subarray(r.offset + r.length, e).toString('hex'),
  ...
  return { runs: runs.length, differing_bytes: runs.reduce((n, r) => n + r.length, 0), size_delta: b.length - a.length, rows };
}
