...
export function cloneAtOwner(session, instruction) {
  ...
  const valid = (p, bytes) =>
    Number.isInteger(p) && p >= sec.offset && p + bytes <= Math.min(b.length, sec.offset + sec.size);
...
function movementDiagnostics(session, placement, sceneRole, trajectory, templateFor) {
  ...
  for (const c of sceneRole?.counterparts ?? [])
    add(
      session.placements.find(p => p.offset === c.offset),
      'counterpart',
    );
...
export function scriptDiagnostics(session, placement) {
  if (!session._scriptDiagnostics) {
    ...
    for (const owner of session.placements.filter(p => p.behavior?.offset != null)) {
      ...
      let decoded;
      try {
        decoded = decodeScript(b, session.graph, session.fixups, offset);
      } catch {
        continue;
      }
      ...
      scripts.set(offset, {
        clones,
        animated: decoded.instructions.some(i => /^(slide value|displace\|)/.test(i.opcode ?? '')),
      });
    }
  ...
  const templateFor = (templates.get(placement.offset) ?? []).map(({ resource, ...s }) => ({
    ...s,
    at_owner: resource.at_owner,
  }));
  return {
    ...(scripts.get(placement.behavior?.offset) ?? { clones: [], animated: false }),
