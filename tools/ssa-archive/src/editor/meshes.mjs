...
function concatenate(units) {
  ...
  for (const u of units) {
    ...
    vo += u.count;
    io += u.triangles.length;
  }
  return {
    descriptors: units.map(u => u.descriptor),
    units: units.length,
    vertex_count,
    triangle_count: icount / 3,
    positions,
    indices,
    bounds: units.length ? { min, max } : null,
  };
}
...
export function partitionGeometry(
  units,
  assignment,
  { maxVertices = 32768, largeSurfaceLimit: limit = Infinity } = {},
) {
  const assigned = new Set([...assignment.byModel.values()].flat());
  const chunks = [],
    unresolved = [];
  const batches = [
    { pending: [], vertices: 0, large: false },
    { pending: [], vertices: 0, large: true },
  ];
  let count = 0;
  const flush = batch => {
  ...
  units.forEach((u, i) => {
    ...
    if (u.kind !== 'interleaved' || u.frac !== 6) {
      unresolved.push({
        ...
        kind: u.kind,
        fraction: u.frac,
        reason: 'world transform not established',
      });
      return;
    }
    const min = [Infinity, Infinity, Infinity],
    ...
    for (let k = 0; k < u.vertices.length; k++) {
      const a = k % 3;
      min[a] = Math.min(min[a], u.vertices[k]);
      max[a] = Math.max(max[a], u.vertices[k]);
    }
    const batch = batches[Math.max(...max.map((x, a) => x - min[a])) > limit ? 1 : 0];
    if (batch.vertices + u.count > maxVertices) flush(batch);
    batch.pending.push(u);
    batch.vertices += u.count;
    count++;
  });
  batches.forEach(flush);
  return {
    assigned_unique: assigned.size,
    unresolved,
...
export function modelMeshes(session) {
  ...
  const paths = new Map(
    session.placements.filter(p => p.model?.offset != null).map(p => [p.model.offset, p.model.path]),
  );
  if (session._meshLibrary) {
    const library = session._meshLibrary,
      out = new Map();
    for (const m of models) {
      const path = paths.get(m),
        original = library.models.get(m);
    ...
    session._meshes = {
      ...library,
      models: out,
      stats: { ...library.stats, models: models.length, with_mesh: out.size },
    };
    return session._meshes;
  }
  const geo = decodeGeometry(buf, graph);
  const a = geo.units.length
    ? assignUnits(buf, graph, geo.units, models, session.fixups)
    : {
        byModel: new Map(models.map(m => [m, []])),
        bounds: new Map(),
        shared: 0,
        world: 0,
        structural: !session.fixups,
      };
  const out = new Map();
  for (const m of models) {
    ...
    out.set(m, {
      ...concatenate(units),
      model: m,
      path: paths.get(m) ?? null,
      fractions: [...new Set(units.map(u => u.frac))],
      bounds: a.bounds.get(m) ?? null,
    });
  }
  ...
  session._meshes = {
    ...
    stats: {
      ...
      with_mesh: out.size,
      draw_units: geo.units.length,
      descriptors: geo.descriptors.length,
      complete: !geo.stop,
      stop: geo.stop ?? null,
      owned: [...a.byModel.values()].reduce((n, l) => n + l.length, 0),
      shared: a.shared,
      world: a.world,
      structural: a.structural,
      assigned_unique: partition.assigned_unique,
      scenery: partition.scenery.units,
...
export function meshesPayload(session) {
  ...
  return {
    ...
    scenery: { ...scenery, chunks: scenery.chunks.map(encode) },
    unresolved,
