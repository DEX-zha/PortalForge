...
export function openLevel(file, fixupsFile) {
  ...
  return { file: path.resolve(file), buf, fixups, graph, table, ptrSet: new Set(fixups.pointer_words), sec: graph.sections[graph.object_section] };
}
...
export function tableRecord(level, offset, buf = level.buf) {
  const i = level.table.indexOf(offset); if (i < 0) return null;
  const end = level.table[i + 1] ?? level.sec.offset + level.sec.size;
  const type = buf.readUInt32BE(offset);
  return { offset, type, size: end - offset, type_name: level.graph.types?.[type]?.name ?? null, id: buf.readUInt32BE(offset + 8) };
}

export function classify(row) {
  const reasons = [];
  if (row.script) reasons.push(`behaviour script ${row.script.path ? row.script.path.replace(/^.*\//, '') : '0x' + row.script.offset.toString(16)} at +0xA8`);
  if (/^CS_|^Cam_|Camera|PortalEntry|_Initialize$/.test(row.name ?? '') || row.layers.some(l => /CS\b|Cams?\b|Camera/.test(l ?? ''))) reasons.push('cutscene/camera marker (not a visible prop)');
...
export function listByLayer(level, opts = {}) {
  ...
  const layers = new Map(); const unlayered = [];
  for (const r of res.rows) {
    r.safety = classify(r);
    if (!r.layers.length) { unlayered.push(r); continue; }
    for (const l of r.layers) { if (!layers.has(l)) layers.set(l, []); layers.get(l).push(r); }
  }
  const out = [...layers.entries()].map(([name, rows]) => ({ name, count: rows.length, scripted: rows.filter(r => r.script).length, static: rows.filter(r => r.safety.safety === 'static').length, rows })).sort((a, b) => b.count - a.count);
...
export function setTransform(level, offset, { position = null, heading = null, scale = null, allowScripted = false }) {
  ...
  if (row.script && !allowScripted) throw Object.assign(new Error(`${row.name} is ${row.safety.safety} (${row.safety.reasons.join('; ')}); pass --allow-scripted to edit it anyway`), { exitCode: 1 });
  const out = Buffer.from(level.buf); const changes = []; const touched = new Set();
  const put = (q, v, label) => { const old = level.buf.readFloatBE(offset + q); out.writeFloatBE(v, offset + q); touched.add(offset + q); changes.push({ field: `+0x${q.toString(16)}`, label, type: 'f32be', old: Math.round(old * 1000) / 1000, new: v, old_hex: level.buf.subarray(offset + q, offset + q + 4).toString('hex'), new_hex: out.subarray(offset + q, offset + q + 4).toString('hex') }); };
  if (position) { if (position.length !== 3 || position.some(v => !Number.isFinite(v))) throw Object.assign(new Error('position needs three finite numbers'), { exitCode: 3 }); put(0x24, position[0], 'x'); put(0x28, position[1], 'y'); put(0x2c, position[2], 'z'); }
  if (heading !== null) put(0x34, heading, 'heading');
  if (scale !== null) put(0xb8, scale, 'scale');
  if (!changes.length) throw Object.assign(new Error('nothing to change (give --pos, --heading or --scale)'), { exitCode: 3 });
  let outside = 0; for (let p = 0; p + 4 <= out.length; p += 4) if (out.readUInt32BE(p) !== level.buf.readUInt32BE(p) && !touched.has(p)) outside++;
  const plan = { mode: 'set-transform', placement: describe(row), changes, validation: { status: outside ? 'INVALID' : 'VALID', failures: outside ? [{ stage: 'validation', reason: `${outside} unexpected word(s) changed` }] : [] }, file_length_unchanged: out.length === level.buf.length, findings: ['igz.placement.type104-record', 'level.prop.scriptset-placement'] };
...
function autoKeep(level, rec, nameField) {
  ...
  for (let q = 12; q + 4 <= rec.size; q += 4) { if (!level.ptrSet.has(rec.offset + q)) continue; const t = level.sec.offset + (level.buf.readUInt32BE(rec.offset + q) & 0x7fffffff); if (t >= rec.offset && t < rec.offset + rec.size) continue; if (COMPANION_TYPES.has(level.buf.readUInt32BE(t))) keep.push(q); }
  return [...new Set(keep)];
}

export function replacePlacement(level, source, victim, { position = null, heading = null, scale = null, keep = 'auto', allowScripted = false, findingId = 'level.prop.scriptset-placement', findingsOpts = {} } = {}) {
  ...
  if (position) { push(base + 0x24, position[0]); push(base + 0x28, position[1]); push(base + 0x2c, position[2]); }
  if (heading !== null) push(base + 0x34, heading);
  if (scale !== null) push(base + 0xb8, scale);
  const r = planReplaceRecord(level.buf, level.fixups, { source: src, victim: vic, keep: keepList, findingId, edits, findingsOpts, extraFindings: ['igz.placement.type104-record'], resolve, refcount });
...
export function formatLayers(res, { limit = 40 } = {}) {
  const lines = [`type-104 placements: ${res.total_type104} in the header table, ${res.listed} listed in ${res.layers.length} layer(s)${res.unlayered.length ? `, ${res.unlayered.length} unlayered` : ''}`, 'flags:  (blank) static   !  scripted / potentially unsafe   ?  marker or no model'];
  let shown = 0;
  for (const L of res.layers) {
    lines.push(`\n[${L.name}]  ${L.count} placement(s): ${L.static} static, ${L.scripted} scripted`);
    for (const r of L.rows) { if (shown++ >= limit) break; lines.push(formatRow(r)); }
    if (shown >= limit) { lines.push(`  ... (limit ${limit}; use --limit or --layer)`); break; }
  }
  return lines.join('\n');
}
...
export function formatRow(r) {
  ...
  return `${flag}0x${r.offset.toString(16)} ${String(r.name ?? '?').slice(0, 30).padEnd(30)} ${r.position ? `(${r.position.map(v => v.toFixed(2)).join(', ')})`.padEnd(28) : '(no position)'.padEnd(28)} h${String(r.heading).padStart(5)} s${String(r.scale).padStart(5)} ${model.padEnd(32)} ${script.padEnd(28)} ${r.safety.safety}`;
}
