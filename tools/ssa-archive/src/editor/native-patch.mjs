...
function compile(additions, { scaleHook = false, probe = false } = {}) {
  if (!Array.isArray(additions) || !additions.length || additions.length > NATIVE_LIMIT)
    throw Error(`Native patch requires 1..${NATIVE_LIMIT} additions`);
  const ids = new Set();
  for (const a of additions) {
    ...
    for (const k of ['source', 'model'])
      if (!Number.isInteger(a[k]) || a[k] < 0 || NATIVE_BASE + a[k] + 0xf8 >= 0x81800000 || a[k] % 4)
    ...
    if (
      probe &&
      a.script != null &&
      (!Number.isInteger(a.script) || a.script < 0 || a.script % 4 || NATIVE_BASE + a.script >= 0x81800000)
    )
  ...
  p.emit(NATIVE_STRIDE);
  p.label('records');
  for (const a of additions) {
    const row = Array(NATIVE_STRIDE / 4).fill(0);
    row[0] = NATIVE_MAGIC;
    row[0x1c / 4] = a.id >>> 0;
    a.position.forEach((v, i) => (row[8 + i] = floatWord(v)));
    row[0x2c / 4] = NATIVE_BASE + a.source;
    row[0x70 / 4] = floatWord(a.heading);
    for (const o of [0x78, 0x7c, 0x80]) row[o / 4] = floatWord(a.scale);
