...
import {ensureLevelEntry,restoreLevelEntry,entrySteps,tutorialSteps,validateSkipIntro,directEntryConfirmed} from './level-entry.mjs';
...
export async function runEditorGame({ patch, archive, mode = 'test', skip_intro = false, figure = null, prediction = '', signal,
  ...
  if(direct){entrySteps(archive);if(!entryServices.confirmed())throw Error('Direct level entry is still being validated. Use normal play or the tutorial test.');}
  ...
  const record = { id, kind: 'EDITOR_PREVIEW', mode, skip_intro, started: new Date().toISOString(), patch: patch.descriptor,
  ...
  try {
    ...
    let entry;
    if(direct){stage='entry-prepare';entry=await entryServices.prepare({game,patch,archive,figure,onProgress});stopped=undefined;}
    ...
    if(direct){stage='entry-restore';onProgress({phase:'restoring-entry'});const restored=await entryServices.restore(game,entry,figure);restoreEntry=restored.restore;record.entry=restored.proof;if(patch.native_additions)await verifyNativeFactory();}
    stage = testing || direct ? 'macro' : 'playing';
    if (testing || direct) {
      const steps = direct ? entrySteps(archive,{skipIntro:skip_intro,interactive:!testing}) : tutorialSteps({skipIntro:skip_intro});
    ...
    try { record.stop = await stop(); }
      ...
      if(restoreEntry&&(!game.pid||record.stop?.stopped)){restoreEntry();record.entry_slot_restored=true;}
    }
