...
export function driftingPath(session, placement) {
  ...
  const b = session.buffer,
    sec = session.graph.sections[session.graph.object_section];
  ...
  if (list === null || list + 24 > sec.offset + sec.size || b.readUInt32BE(list) !== 147)
    return invalid('unconfirmed waypoint list');
  const count = b.readUInt32BE(list + 8),
    start = readRef(list + 20);
  if (
    !count ||
    count > 1024 ||
    b.readUInt32BE(list + 12) !== count ||
    b.readUInt32BE(list + 16) !== (0x80000000 | (count * 4)) >>> 0 ||
    start === null ||
    start + count * 4 > sec.offset + sec.size
  )
    return invalid('invalid waypoint array');
  const points = [];
  for (let i = 0; i < count; i++) {
    const p = readRef(start + i * 4);
    if (p === null || p + 56 > sec.offset + sec.size || b.readUInt32BE(p) !== 148)
      return invalid('unconfirmed waypoint record');
    const position = [0, 1, 2].map(a => b.readFloatBE(p + 0x20 + a * 4));
    if (!position.every(Number.isFinite) || points.some(v => v.offset === p))
      return invalid('invalid waypoint positions');
...
export function translatedPathWords(session, placement, position) {
  ...
  if (!path) return [];
  if (!path.supported)
    throw Object.assign(new Error('The drifting path cannot be translated safely: ' + path.reason), {
      error: 'UNSUPPORTED_TRAJECTORY',
    });
  const b = session.buffer;
  const delta = position.map((v, a) => Math.fround(v) - b.readFloatBE(placement.offset + 0x24 + a * 4));
  return path.points.flatMap(p =>
    p.position.map((old, a) => {
      const value = Math.fround(old + delta[a]);
      if (!Number.isFinite(value))
        throw Object.assign(new Error('Trajectory translation exceeds float32 range'), { error: 'BAD_VALUE' });
