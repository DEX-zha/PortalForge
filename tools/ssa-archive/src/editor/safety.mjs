...
export const TRACK_BOUND = [
  { match: /PushBlock/i, why: 'push blocks resolve a track, waypoints and switches at STARTUP (PushBlock_Template.ai, 1415 instructions); two boots froze in game after one was placed on the start island', finding: 'level.pushblock.track-dependency' },
];
...
export function assessPlacement(placement, { hasRuntimeMap = false } = {}) {
  ...
  if (placement.behavior) {
    ...
    if (track) rules.push({ id: 'TRACK_BOUND_BEHAVIOUR', severity: 'critical', finding: track.finding,
    ...
    else rules.push({ id: 'SCRIPTED_PLACEMENT', severity: 'medium', finding: 'level.pushblock.track-dependency',
  ...
  if (!hasRuntimeMap) rules.push({ id: 'NO_RUNTIME_MAP', severity: 'info', finding: 'igz.placement.type104-record',
    message: 'this level has no runtime fixup map, so pointer fields are structural only. The structural resolver reproduces the runtime answer on the tutorial at 100%, but that is calibration, not proof for this file.' });
...
export function assessReplacement(source, victim, { hasRuntimeMap = false, sharedInVictim = [], spanMatch = true, copied = null } = {}) {
  const rules = [];
  if (!spanMatch) rules.push({ id: 'SPAN_MISMATCH', severity: 'blocking', finding: 'igz.loader.head-span-count-walk',
  ...
  for (const s of sharedInVictim) {
    ...
    const what = s.path ? `${isModel ? 'the model record' : 'the record'} at 0x${s.offset.toString(16)} (${s.path.replace(/^.*\//, '')})` : `the record at 0x${s.offset.toString(16)}`;
    if (s.users > 1 && isModel) rules.push({ id: 'SHARED_MODEL_REWRITE', severity: 'critical', finding: 'igz.placement.shared-model-record',
    ...
    else if (s.users > 1) rules.push({ id: 'SHARED_RECORD_REWRITE', severity: 'critical', finding: 'igz.placement.shared-model-record',
    ...
    else rules.push({ id: 'OWN_RECORD_REWRITE', severity: 'info', finding: 'igz.placement.shared-model-record',
  ...
  const vic = assessPlacement(victim, { hasRuntimeMap }).filter(r => r.id === 'TRACK_BOUND_BEHAVIOUR' || r.id === 'SCRIPTED_PLACEMENT');
  for (const r of vic) rules.push({ ...r, id: 'VICTIM_' + r.id, severity: r.severity === 'critical' ? 'high' : 'info',
    message: 'victim slot: ' + r.message + ' Its script pointer is overwritten by the source unless kept.' });
  if (!hasRuntimeMap) rules.push({ id: 'NO_RUNTIME_MAP', severity: 'info', finding: 'igz.placement.type104-record',
    message: 'no runtime fixup map for this level: the pointer fields rebased by the planner are identified structurally.' });
  return rules.sort((a, b) => rank(a.severity) - rank(b.severity));
}
...
export function formatRules(rules) {
  ...
  return rules.map(r => `  [${r.severity.toUpperCase().padEnd(8)}] ${r.id}: ${r.message}${r.finding ? `  (${r.finding})` : ''}`).join('\n');
}
