...
export async function installNativePatch(native, { directory = profile, occupied = portOccupied, compiler = compileNativePatch } = {}) {
  ...
  let fd;
  try { fd = fs.openSync(lease, 'wx'); } catch { throw Error('A native patch session already owns this research profile.'); }
  ...
  const restore = () => {
    for (let i = 0; i < files.length; i++) {
      if (previous[i] === null) { if (fs.existsSync(files[i])) fs.unlinkSync(files[i]); }
      else fs.writeFileSync(files[i], previous[i]);
    }
    fs.closeSync(fd); fs.unlinkSync(lease);
  };
  try {
    fs.writeFileSync(fd, JSON.stringify({ pid: process.pid, previous: previous.map(b => b?.toString('base64') ?? null) }));
    const game = previous[1]?.toString('utf8') ?? '';
    if (/^\s*\[Gecko/m.test(game)) throw Error('The research profile already contains Gecko codes; keep them in a separate profile before using native additions.');
    let config = previous[0]?.toString('utf8') ?? '';
    const lines = config.split(/\r?\n/); let start = lines.findIndex(l => l.trim() === '[Core]');
    if (start < 0) { lines.push('[Core]'); start = lines.length - 1; }
    ...
    if (key >= 0) lines[key] = 'EnableCheats = True'; else lines.splice(end, 0, 'EnableCheats = True');
...
export async function verifyNativeInstances(additions, read = readNativeBytes) {
  const memory = await read(0x80001800, 0x1800), rows = [];
  const valid = p => p % 4 === 0 && p >= 0x80003000 && p + 0xf8 <= 0x81800000;
  for (const a of additions) {
    const matches = [];
    for (let off = 0; off + NATIVE_STRIDE <= memory.length; off += 4) {
      if (memory.readUInt32BE(off) === NATIVE_MAGIC && memory.readInt32BE(off + 0x1c) === a.id && memory.readUInt32BE(off + 0x2c) === NATIVE_BASE + a.source) matches.push(off);
    }
...
export async function verifyNativeLifecycle(additions, observations, verify = verifyNativeInstances) {
  try { return {...await verify(additions), lifecycle:'present_at_end'}; }
  catch (error) {
