// The HTML page that shows one addition test: its verdict and every tutorial capture of every run, in order.
...
function runSection(run, runIndex, familyKey) {
  const captures = (run.screenshots ?? [])
    ...
    .filter(capture => /tutorial/.test(capture.file))
    .map(
      capture =>
        `<p>${escapeHtml(path.basename(capture.file))}</p>` +
        `<img loading="lazy" alt="Tutorial capture, run ${runIndex + 1}" src="/api/addition-report/${familyKey}/shot/${runIndex}/${capture.shotIndex}">`,
    )
    .join('');
  return `<h2>Run ${runIndex + 1}: ${escapeHtml(run.status)}</h2>${captures}`;
}
...
export function renderAdditionReport(report, record, familyKey) {
  return (
    ...
    `<p>Visual: ${escapeHtml(report.visual)}. Gameplay: ${escapeHtml(report.gameplay)}. A technical pass alone does not enable Add.</p>` +
