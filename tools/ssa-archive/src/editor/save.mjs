...
export function buildSavePlan(session) {
  ...
  let additions = [];
  try {
    additions = currentAdditionRecipe(session);
  } catch (e) {
    failures.push({ stage: 'additions', reason: e.message });
  }
  ...
  if (!session.edits.length && !additions.length)
    failures.push({ stage: 'edits', reason: 'nothing to save: no edit has been applied' });
    ...
    if (e.kind === 'replace') {
      for (const w of e.words)
        changes.push({
          target: e.target,
          attribute: 'replace',
          field: `+0x${(w.offset - e.target).toString(16)}`,
          old: null,
          new: null,
          old_hex: original ? hexAt(original, w.offset) : null,
          new_hex: hexAt(session.buffer, w.offset),
        });
      continue;
    }
    for (const a of e.attributes) {
      ...
      words.forEach((w, i) =>
        changes.push({
          target: e.target,
          attribute: a,
          field: `+0x${(w - e.target).toString(16)}`,
          old: olds[i],
          new: values[i],
          old_hex: original ? hexAt(original, w) : null,
          new_hex: hexAt(session.buffer, w),
        }),
      );
    }
    for (const w of e.dependent_words ?? [])
      changes.push({
        target: e.target,
        attribute: 'trajectory',
        field: `+0x${(w.offset - e.target).toString(16)}`,
        old: original ? original.readFloatBE(w.offset) : null,
        new: session.buffer.readFloatBE(w.offset),
        old_hex: original ? hexAt(original, w.offset) : null,
        new_hex: hexAt(session.buffer, w.offset),
      });
  }
...
function dedupe(changes) {
  ...
  for (const c of changes) {
    ...
    if (byField.has(key)) {
      const first = byField.get(key);
      byField.set(key, { ...c, old: first.old, old_hex: first.old_hex });
...
function* allowedWordsFor(offset, attribute) {
  if (attribute === 'position') {
    yield offset + FIELDS.position;
    yield offset + FIELDS.position + 4;
    yield offset + FIELDS.position + 8;
...
export function save(session, { out = null } = {}) {
  ...
  if (target.toLowerCase() === path.resolve(session.file).toLowerCase())
    fail('SOURCE_OVERWRITE', 'save to a separate file to preserve the opened source');
  ...
  try {
    ...
    plan.failures.push({
      stage: 'write',
      reason: `${target} could not be written: ${e.code === 'ENOENT' ? 'the directory does not exist' : e.message}`,
    });
  ...
  try {
    saveAdditionSidecar(session, target, sha256);
  } catch (e) {
    plan.status = 'INVALID';
    plan.failures.push({ stage: 'addition-write', reason: e.message });
    return { plan, written: null };
  }
  session.dirty = false;
  ...
  session.lastSave = {
    ...
    plan,
    sha256,
    additions: plan.native_additions,
    additions_sha256: additionsDigest(plan.native_additions),
...
export function patch(session, { deps = {} } = {}) {
  ...
  if (session.locked)
    fail(
      'SESSION_LOCKED',
      `a launched run may still be reading ${session.lock?.patch_dir}; record what you saw before rebuilding it`,
    );
  if (
    hash(session.buffer) !== session.lastSave.sha256 ||
    !fs.existsSync(session.lastSave.file) ||
    hash(fs.readFileSync(session.lastSave.file)) !== session.lastSave.sha256
  )
    fail('UNSAVED_CHANGES', 'save the current edits before building their patch');
  if (additionsDigest(currentAdditionRecipe(session)) !== (session.lastSave.additions_sha256 ?? additionsDigest([])))
    fail('UNSAVED_CHANGES', 'Save the current added objects before building their patch.');
  if (session.lastSave.additions_sha256) {
    let sidecar;
    try {
      sidecar = JSON.parse(fs.readFileSync(session.lastSave.file + '.portalforge.json', 'utf8'));
    } catch {
      fail('STALE_ADDITIONS', 'The saved addition file is missing or unreadable.');
    }
    if (
      sidecar.base_sha256 !== session.lastSave.sha256 ||
      additionsDigest(sidecar.additions) !== session.lastSave.additions_sha256
    )
      fail('STALE_ADDITIONS', 'The saved addition file changed. Save the scene again.');
  }
  ...
  session.lastPatch = {
    ...
    native_additions,
    rebuilt_sha256: result.rebuilt_sha256 ?? null,
    at: new Date().toISOString(),
  };
...
export async function launch(
  session,
  { prediction = '', figure = null, repeat = 1, mode = null, skip_intro = false, deps = {}, wait = false } = {},
) {
  if (mode !== null && !['test', 'play', 'direct-test', 'direct-play'].includes(mode))
    fail('BAD_MODE', 'choose test, play, direct-test or direct-play');
  validateSkipIntro(session.archive, mode ?? 'test', skip_intro);
  if (!mode && !String(prediction).trim())
    fail(
      'PREDICTION_REQUIRED',
      'state what should be visible before the game starts: an experiment without a prediction cannot be judged',
    );
  ...
  if (additionsDigest(currentAdditionRecipe(session)) !== (session.lastPatch.additions_sha256 ?? additionsDigest([])))
    fail('STALE_PATCH', 'Added objects changed: save and rebuild the patch first.');
  ...
  prediction =
    ...
    session.lastSave.plan.changes
      .map(c => `0x${c.target.toString(16)} ${c.attribute} ${c.field}: ${c.old_hex} -> ${c.new_hex}`)
      .join('; ') ||
    'Verify the saved level in game';
  const run =
    deps.run ??
  ...
  const started = Promise.resolve()
    ...
    .then(record => {
      current.experiment_id = record?.id ?? null;
    ...
    .catch(e => {
      session.lastLaunch.error = e.message;
      return null;
    })
    .finally(() => {
      current.running = false;
      current.finished = new Date().toISOString();
      if (mode) {
...
export function observe(session, { experiment_id, observed, matched, deps = {} } = {}) {
  ...
  if (!String(observed ?? '').trim())
    fail('OBSERVATION_REQUIRED', 'say what was actually seen, even when it is nothing');
  session.lastLaunch.observed = String(observed).trim();
  session.lastLaunch.matched = !!matched;
  if (deps.judge)
    session.lastLaunch.judged = deps.judge({
      id: session.lastLaunch.experiment_id,
      observed: session.lastLaunch.observed,
      matched: !!matched,
    });
  session.lock = null;
  session.locked = false;
