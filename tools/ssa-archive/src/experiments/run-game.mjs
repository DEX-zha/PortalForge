...
export const gateStatus = name => { try { return JSON.parse(fs.readFileSync(path.join(root, 'docs', `${name.toLowerCase()}-status.json`), 'utf8')); } catch { return { status: 'UNKNOWN' }; } };
const sleep = ms => new Promise(r => setTimeout(r, ms));

export class GameSession {
  constructor(log = console.log) { this.log = log; this.client = null; this.pid = null; this.logOffset = 0; }
  async connect() {
    this.client = new Client({ name: 'portalforge-ssa-archive', version: '0.1.0' });
    await this.client.connect(new StdioClientTransport({ command: process.execPath, args: [path.join(mcpDir, 'server.mjs')], stderr: 'inherit' }));
    return this;
  }
  ...
  async json(name, args = {}) { return JSON.parse(this.text(await this.call(name, args))[0]); }
  logSince(offset = this.logOffset) {
    if (!fs.existsSync(profileLog)) return '';
    const size = fs.statSync(profileLog).size; if (size <= offset) return '';
    const fd = fs.openSync(profileLog, 'r');
    try { const b = Buffer.alloc(size - offset); fs.readSync(fd, b, 0, b.length, offset); return b.toString('utf8'); } finally { fs.closeSync(fd); }
  }
  monitorLines(discPath) {
    const needle = discPath.replace(/^\/+/, '');
    return this.logSince().split(/\r?\n/).filter(l => l.includes('FileMon') && l.includes(needle)).map(l => l.trim());
  }
  // Normalised size token ("52733 kB"): Dolphin prints thin-space thousands separators.
  static monitorSize(line) { const m = /\]:\s+([\d\s ]+)kB\s/.exec(line); return m ? m[1].replace(/[\s ]+/g, '') + ' kB' : null; }
  static normSize(s) { return String(s ?? '').replace(/[\s ]+/g, ''); }
  ...
  async stop() {
    ...
    try { const r = await this.json('dolphin_stop'); if (r.stopped) this.log(`stopped pid ${r.pid} forced=${r.forced}`); this.pid = null; return r; } catch (e) { return { error: e.message }; }
  }
  ...
  static isTimeout(e) { return /timed out|bridge timeout/i.test(e.message ?? ''); }
  async waitSeconds(seconds, { stallLimit = 8 } = {}) {
    const deadline = Date.now() + seconds * 1000;
    let stalls = 0;
    while (Date.now() < deadline) {
      try { await this.call('dolphin_frame_advance', { frames: 120 }); stalls = 0; }
      catch (e) {
  ...
  async callWithRetry(name, args, attempts = 8) {
    for (let i = 1; ; i++) {
      try { return await this.call(name, args); }
      catch (e) { if (!GameSession.isTimeout(e) || i >= attempts) throw e; this.log(`  ${name}: emulation stalled (${i}/${attempts}), retrying`); await sleep(3000); }
    }
  }
  ...
  async press(button, frames = 3) { await this.callWithRetry('dolphin_hold_wii_input', { buttons: { [button]: true }, frames }); }
  async nunchuk(stick, frames = 60) { await this.callWithRetry('dolphin_hold_wii_input', { nunchuk: stick, frames }); }
  // A missed screenshot is recorded as null rather than aborting the run; the trace shows the gap.
  async screenshot(label) {
    let t;
    try { t = this.text(await this.callWithRetry('dolphin_screenshot')); }
    catch (e) { if (!GameSession.isTimeout(e)) throw e; this.log(`  screenshot ${label ?? ''} skipped: ${e.message.split('\n')[0]}`); return null; }
  ...
  async runScript(steps, { labelPrefix = 'step', onShot = null, figure = null, onStep = null } = {}) {
    ...
    for (let i = 0; i < steps.length; i++) {
      const s = { ...steps[i] }; const t0 = Date.now(); let note = null;
      if (s.figure === '@figure') { if (!figure) { push({ step: i, ...s, skipped: 'no --figure given' }); continue; } s.figure = figure; }
