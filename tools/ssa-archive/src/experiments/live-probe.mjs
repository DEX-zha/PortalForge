  ...
  for (const [start, end] of regions) {
    ...
    for (let address = start; address < end; address += 65536) {
      const bytes = Buffer.from(
        await bridgeCall('memory.read_bytes', [address, Math.min(65536, end - address)], 30000),
        'hex',
      );
      const combined = Buffer.concat([previous, bytes]);
      needles.forEach((n, i) => {
        let o = combined.indexOf(n.buf);
        while (o !== -1) {
          matches[i].push(address - previous.length + o);
          o = combined.indexOf(n.buf, o + 1);
        }
      });
...
export async function snapshot(start, end, log = null) {
  ...
  for (let address = start; address < end; address += 65536) {
    parts.push(
      Buffer.from(await bridgeCall('memory.read_bytes', [address, Math.min(65536, end - address)], 30000), 'hex'),
    );
...
export function diffFloats(a, b, base, { minDelta = 0.2, maxDelta = 500, maxAbs = 1e5 } = {}) {
  ...
  for (let i = 0; i + 2 < changed.length; i++) {
    if (changed[i + 1].address === changed[i].address + 4 && changed[i + 2].address === changed[i].address + 8)
      triples.push({
        address: '0x' + changed[i].address.toString(16),
        before: [changed[i].before, changed[i + 1].before, changed[i + 2].before].map(v => +v.toFixed(3)),
        after: [changed[i].after, changed[i + 1].after, changed[i + 2].after].map(v => +v.toFixed(3)),
      });
  }
...
export async function ramDiff({
  label = 'ram-diff',
  game = gameFromConfig(),
  stateSlot = 6,
  region = [0x80000000, 0x81800000],
  moveFrames = 90,
  figure = null,
  log = console.log,
} = {}) {
  ...
  const report = {
    ...
    region: region.map(r => '0x' + r.toString(16)),
    figure,
  };
  try {
    ...
    await session.nunchuk({ StickX: 1, StickY: 0 }, moveFrames);
    await session.waitSeconds(3);
    ...
    const ab = diffFloats(a, b, region[0]),
      bc = diffFloats(b, c, region[0]);
    ...
    report.candidates = ab.triples
      .filter(t => back.has(t.address))
      .map(t => {
        const r = back.get(t.address);
        const dx = t.after.map((v, i) => v - t.before[i]);
        const dr = r.after.map((v, i) => v - r.before[i]);
        const k = dx.map(Math.abs).indexOf(Math.max(...dx.map(Math.abs)));
        return {
          ...t,
          delta_right: dx.map(v => +v.toFixed(3)),
          delta_back: dr.map(v => +v.toFixed(3)),
          dominant_axis: k,
          reverses: Math.sign(dx[k]) === -Math.sign(dr[k]),
        };
      })
      .filter(c => c.reverses);
    report.changed_ab = ab.changed;
    report.changed_bc = bc.changed;
    report.triples_ab = ab.triples.length;
    report.status = 'DONE';
    log(`candidates: ${report.candidates.length}`);
    report.candidates
      .slice(0, 10)
      .forEach(c =>
        log(
          `  ${c.address} ${JSON.stringify(c.before)} -> ${JSON.stringify(c.after)} right=${JSON.stringify(c.delta_right)} back=${JSON.stringify(c.delta_back)}`,
        ),
      );
  } catch (e) {
    report.status = 'FAIL';
...
export async function ptrScan({
  ...
  try {
    ...
    for (const [s, e] of regions) {
      log(`snapshot 0x${s.toString(16)}..0x${e.toString(16)}`);
      snaps.push({ start: s, buf: await snapshot(s, e, log) });
    }
    const readAt = (addr, n) => {
      for (const s of snaps)
        if (addr >= s.start && addr + n <= s.start + s.buf.length)
          return s.buf.subarray(addr - s.start, addr - s.start + n);
      return null;
    };
    ...
    if (dumpSection) {
      ...
      report.region_dumps = [];
      for (const s of snaps) {
        const f = path.join(evidence, `${label}-mem-${s.start.toString(16)}.bin`);
        fs.writeFileSync(f, s.buf);
        report.region_dumps.push({ start: '0x' + s.start.toString(16), file: f, bytes: s.buf.length });
        log(`region dump ${f} (${s.buf.length} bytes)`);
      }
    }
    ...
    for (const t of targets) {
      const isAddr = t >= 0x80000000;
      const address = isAddr ? t : base + t;
      const fileOffset = isAddr ? t - base : t;
      const entry = {
        target: '0x' + t.toString(16),
        address: '0x' + address.toString(16),
        file_offset: fileOffset,
        object: graph ? matchAddress(graph, address, { base }).object : null,
        resident_diff: null,
        referrers: [],
      };
      if (fileBuf && graph) {
        ...
        if (live) {
          const diffs = [];
          for (let q = 0; q + 4 <= size; q += 4) {
            const a = fileBuf.readUInt32BE(fileOffset + q),
              b = live.readUInt32BE(q);
            if (a !== b)
              diffs.push({
                field: '+0x' + q.toString(16),
                file: '0x' + a.toString(16),
                live: '0x' + b.toString(16),
                live_minus_base: b >= base && b < base + fileBuf.length ? '0x' + (b - base).toString(16) : null,
              });
          }
          entry.resident_diff = {
            compared_bytes: size,
            changed_words: diffs.length,
            words: diffs.slice(0, 64),
            identical_floats_at_0x94:
              obj && obj.size > 0xa0 ? fileBuf.readFloatBE(fileOffset + 0x94) === live.readFloatBE(0x94) : null,
          };
        } else entry.resident_diff = { error: 'address outside the snapshot' };
      }
      const needle = Buffer.alloc(4);
      needle.writeUInt32BE(address >>> 0);
      for (const s of snaps) {
        let o = s.buf.indexOf(needle);
        while (o !== -1 && entry.referrers.length < 200) {
          const at = s.start + o;
          const ref = { at: '0x' + at.toString(16), aligned: o % 4 === 0, in_section: inSection(at) };
          if (ref.in_section && graph) {
            const m = matchAddress(graph, at, { base });
            ref.file_offset = m.file_offset;
            ref.object = m.object;
            ref.field = m.field;
          } else {
            const c0 = Math.max(s.start, at - contextBytes / 2);
            ref.context_at = '0x' + c0.toString(16);
            ref.context = s.buf.subarray(c0 - s.start, c0 - s.start + contextBytes).toString('hex');
          }
          entry.referrers.push(ref);
          o = s.buf.indexOf(needle, o + 1);
        }
      }
      log(
        `target ${entry.target}${entry.object ? ' ' + entry.object.type_name + '@0x' + entry.object.offset.toString(16) : ''}: ${entry.referrers.length} runtime referrer(s), ${entry.resident_diff?.changed_words ?? '?'} changed word(s)`,
      );
    ...
    for (const hex of patterns) {
      ...
      for (const s of snaps) {
        let o = s.buf.indexOf(needle);
        while (o !== -1 && hits.length < 100) {
          const at = s.start + o;
          const h = { at: '0x' + at.toString(16), in_section: inSection(at) };
          if (h.in_section && graph) {
            const m = matchAddress(graph, at, { base });
            h.object = m.object;
            h.field = m.field;
          } else h.context = s.buf.subarray(Math.max(0, o - contextBytes / 2), o + contextBytes / 2).toString('hex');
          hits.push(h);
          o = s.buf.indexOf(needle, o + 1);
        }
      }
...
export async function liveProbe({
  ...
  try {
    await session.launch(game, label);
    report.trace = await session.runScript(readScript(script), {
      labelPrefix: label,
    ...
    if (saveSlot) {
      try {
        report.saved_state = await session.saveState(saveSlot);
      } catch (e) {
        report.saved_state = { error: e.message };
      }
    }
    if (patterns.length) {
      log('scanning RAM for ' + patterns.length + ' pattern(s)');
      report.scan = await scanRam(patterns, { log });
    }
    for (const poke of pokes) {
      let address = poke.address;
      const m = /^match:(\d+):(-?\d+)$/.exec(String(address));
      if (m) {
        const first = report.scan?.[Number(m[1])]?.matches?.[0];
        if (!first) {
          report.poke_results.push({ ...poke, skipped: 'pattern not found' });
          continue;
        }
        address = parseInt(first, 16) + Number(m[2]);
      }
      ...
      const write = await session
        .json('dolphin_write_float', { address, bits: 32, value: poke.value })
      ...
      report.poke_results.push({
        address: '0x' + address.toString(16),
        value: poke.value,
        before,
        write,
        after,
        shot_before: shotBefore,
        shot_after: shotAfter,
      });
      log(
        `  poke 0x${address.toString(16)} = ${poke.value}: before ${JSON.stringify(before)} after ${JSON.stringify(after)}`,
      );
    }
    report.status = 'DONE';
  } catch (e) {
