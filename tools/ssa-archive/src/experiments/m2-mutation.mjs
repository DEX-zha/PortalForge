...
const TYPES = { f32le: [4, (b, o, v) => b.writeFloatLE(v, o), (b, o) => b.readFloatLE(o)], f32be: [4, (b, o, v) => b.writeFloatBE(v, o), (b, o) => b.readFloatBE(o)],
  u32le: [4, (b, o, v) => b.writeUInt32LE(v >>> 0, o), (b, o) => b.readUInt32LE(o)], u32be: [4, (b, o, v) => b.writeUInt32BE(v >>> 0, o), (b, o) => b.readUInt32BE(o)],
  u16le: [2, (b, o, v) => b.writeUInt16LE(v, o), (b, o) => b.readUInt16LE(o)], u16be: [2, (b, o, v) => b.writeUInt16BE(v, o), (b, o) => b.readUInt16BE(o)], u8: [1, (b, o, v) => b.writeUInt8(v, o), (b, o) => b.readUInt8(o)] };
...
export async function prepareMutation({ archive, entry, offset, type, value, game, id, log = console.log }) {
  ...
  let data;
  if (e.compression === 'LZMA_CHUNKED') { await decodeWorkspace(wsDir); m = readManifest(wsDir); data = fs.readFileSync(path.join(wsDir, m.entries[entry].decoded_file)); }
  ...
  const oldValue = read(data, offset), old_hex = data.subarray(offset, offset + width).toString('hex');
  const mutated = Buffer.from(data); write(mutated, offset, value);
  const new_hex = mutated.subarray(offset, offset + width).toString('hex');
  const replFile = path.join(wsDir, `mutated-entry-${entry}.bin`); fs.writeFileSync(replFile, mutated);
  ...
  const rebuiltPath = path.join(wsDir, `rebuilt-m2${path.extname(archive)}`); fs.writeFileSync(rebuiltPath, built.buffer);
  log(`mutation: entry ${entry} (${e.name}) @${offset} ${type} ${oldValue} -> ${value} (${old_hex} -> ${new_hex}); ${built.strategy}; ${validation.status}; diff ${diff.regions.length} regions`);
  return { source, original_sha256: sha256(original), original_size: original.length, workspace: wsDir, rebuilt: rebuiltPath, rebuilt_sha256: sha256(built.buffer), rebuilt_size: built.buffer.length,
    strategy: built.strategy, validation, diff: { size_delta: diff.size_delta, unclassified_bytes: diff.unclassified_bytes, summary: diff.summary, regions: diff.regions.length },
    mutation: { entry_index: entry, entry_name: e.name, offset, type, old_hex, new_hex, old_value: oldValue, new_value: value } };
}
...
async function observe(session, label, target, { script, figure, archive, watch }) {
  ...
  for (const w of watch) {
    try { const t = session.text(await session.call('dolphin_read_range', { address: w, length: 16 }))[0]; run.memory_reads.push({ address: w, bytes_hex: t }); } catch (e) { run.memory_reads.push({ address: w, bytes_hex: 'ERROR ' + e.message }); }
  }
...
export async function runM2({ archive, entry, offset, type, value, predict, findingId = null, repeat = 2, figure = null, script = defaultScript, watch = [], game = gameFromConfig(), skipControl = false, log = console.log }) {
  if (gateStatus('m0').status !== 'PASS') { const e = new Error('M0 is not PASS'); e.exitCode = 2; throw e; }
  if (gateStatus('m1').status !== 'PASS') log('WARNING: M1 is not PASS; an M2 result cannot be trusted until a rebuilt archive is known to load');
  const id = `m2-${path.basename(archive, path.extname(archive)).toLowerCase()}-e${entry}-o${offset}-${Date.now()}`;
  const record = { id, kind: 'M2_MUTATION', started: new Date().toISOString(), inputs: { game: path.resolve(game), archive_disc_path: archive, figure, input_script: path.resolve(script), watch }, runs: [], status: 'UNKNOWN' };
  ...
  try {
    ...
    Object.assign(record.inputs, { original_sha256: prep.original_sha256, rebuilt_sha256: prep.rebuilt_sha256,
      mutation: { entry_index: entry, offset, type, old_hex: prep.mutation.old_hex, new_hex: prep.mutation.new_hex, predicted_effect: predict, ...(findingId ? { finding_id: findingId } : {}) } });
    ...
    const patch = buildPatchWorkspace({ experimentId: id, game, replacements: [{ disc_path: archive, file: prep.rebuilt, original: prep.source }], outDir: path.join(local, 'patches', id), force: true, displayName: `M2 ${archive} e${entry}@${offset}` });
    ...
    record.control = skipControl ? { label: 'skipped', pid: 0, started: '', finished: '', monitor_lines: [], screenshots: [], crash_or_load_error: false, note: 'control skipped by flag' }
      : await observe(session, `${id}-control`, game, { script, figure, archive, watch });
    for (let i = 1; i <= repeat; i++) record.runs.push(await observe(session, `${id}-run${i}`, patch.descriptor, { script, figure, archive, watch }));
    ...
    record.notes = crashed ? 'a run crashed or lost the bridge' : `awaiting human judgement: run "ssa-archive experiment m2-judge --id ${id} --run <n> --observed \"...\" --match yes|no" for each of ${repeat} run(s)`;
    return record;
