...
export async function explore({ script = defaultScript, figure, label = 'explore', game = gameFromConfig(), target = null } = {}) {
  ...
  try {
    ...
    report.steps = await session.runScript(readScript(script), { labelPrefix: label, onShot: f => report.shots.push(f), figure });
