...
export function parseIgzHeader(buf) {
  const issues = [];
  if (buf.length < 0x20) { issues.push(failure(0, 'file', buf.length, '>= 32 bytes', 'BAD_IGZ_MAGIC')); return { issues }; }
  const magic = buf.readUInt32BE(0), version = buf.readUInt32BE(4);
  if (magic !== IGZ_MAGIC) issues.push(failure(0, 'magic', '0x' + magic.toString(16), '0x' + IGZ_MAGIC.toString(16), 'BAD_IGZ_MAGIC'));
  ...
  for (let o = 0x10; o + 16 <= Math.min(buf.length, 0x800); o += 16) {
    ...
    if (offset === 0) break;
    sections.push({ index: sections.length, offset, size: buf.readUInt32BE(o + 4), align: buf.readUInt32BE(o + 8), tag: buf.readUInt32BE(o + 12), table_offset: o });
  }
  const sorted = [...sections].sort((a, b) => a.offset - b.offset);
  for (let i = 1; i < sorted.length; i++) {
    if (sorted[i - 1].offset + sorted[i - 1].size > sorted[i].offset) issues.push(failure(sorted[i].table_offset, `sections[${sorted[i].index}]`, sorted[i].offset, `>= ${sorted[i - 1].offset + sorted[i - 1].size}`, 'SECTION_OVERLAP'));
  }
  if (sorted.length) {
    const last = sorted[sorted.length - 1];
    if (last.offset + last.size !== buf.length) issues.push(failure(last.table_offset, `sections[${last.index}].end`, last.offset + last.size, buf.length, 'SECTION_END_MISMATCH'));
  ...
  if (sections[0] && sections[0].offset + 0x34 <= buf.length) {
    second = { offset: sections[0].offset, words: Array.from({ length: 13 }, (_, i) => buf.readUInt32BE(sections[0].offset + 4 * i)) };
    if (second.words[0] !== IGZ_MAGIC) issues.push(failure(sections[0].offset, 'section0.magic', '0x' + second.words[0].toString(16), '0x' + IGZ_MAGIC.toString(16), 'BAD_IGZ_MAGIC'));
  }
