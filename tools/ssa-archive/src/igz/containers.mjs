...
export function decodeContainers(buf, graph, { minMembers = 1, minResolved = 0.5 } = {}) {
  ...
  for (const o of graph.objects) {
    ...
    for (let q = 12; q + 12 <= o.size; q += 4) {
      const count = buf.readUInt32BE(o.offset + q),
        cap = buf.readUInt32BE(o.offset + q + 4);
      ...
      for (let k = 8; k <= 16 && q + k + 4 <= o.size; k += 4) {
        ...
        for (let i = 0; i < count; i++) {
          const v = buf.readUInt32BE(arr + 4 * i);
          if (isMemberWord(v)) {
            resolved++;
            members.push(objSec.offset + v);
          } else if (v === 0xffffffff) members.push(null);
          else members.push(undefined);
        }
        if (resolved / count >= minResolved) {
          containers.push({
            container: { offset: o.offset, type_name: o.type_name, id: o.id },
            kind: 'pointer_list',
            count_field: q,
            count,
            capacity: cap,
    ...
    for (let q = 12; q + 4 <= o.size; q += 4) {
      ...
      if (isMemberWord(v)) {
        if (!run.length) runStart = q;
        run.push(objSec.offset + v);
      } else {
        if (run.length >= 3)
          containers.push({
            container: { offset: o.offset, type_name: o.type_name, id: o.id },
            kind: 'inline_array',
            array_field: runStart,
            count: run.length,
            members: run,
          });
        run = [];
...
export function ownerChain(buf, graph, targetOffset, { depth = 3 } = {}) {
  const levels = [];
  let frontier = [targetOffset];
  const seen = new Set([targetOffset]);
  for (let d = 0; d < depth && frontier.length; d++) {
    const level = [];
    for (const t of frontier) {
      for (const h of reverseReferences(buf, graph, t)) {
        level.push({ target: t, ...h });
        if (h.referrer && !seen.has(h.referrer.offset)) {
          seen.add(h.referrer.offset);
        }
      }
    }
    levels.push(level);
