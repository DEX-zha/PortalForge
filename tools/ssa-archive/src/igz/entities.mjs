...
export function positionCandidates(buf, object, { min = -20000, max = 20000, minAbs = 0.5 } = {}) {
  ...
  for (let q = 12; q + 12 <= object.size; q += 4) {
    ...
    if (q + 24 <= object.size) {
      const n = [12, 16, 20].map(k => buf.readFloatBE(object.offset + q + k));
      if (n.every(Number.isFinite) && isBoxPair(t, n)) flags.push('box_min');
    }
    if (q >= 24) {
      const p = [-12, -8, -4].map(k => buf.readFloatBE(object.offset + q + k));
      if (p.every(Number.isFinite) && isBoxPair(p, t)) flags.push('box_max');
    }
  ...
  for (const o of graph.objects) {
    for (const c of positionCandidates(buf, o, { minAbs: 0 })) {
      ...
      if (d <= tol)
        rows.push({
          ...c,
          object: { offset: o.offset, type_name: o.type_name, id: o.id },
...
export function fieldStatistics(buf, graph, { minAbs = 1 } = {}) {
  ...
  for (const o of graph.objects) {
    for (const c of positionCandidates(buf, o, { minAbs })) {
      ...
      e.count++;
      if (c.flags.includes('box_min')) e.boxes++;
      stats.set(key, e);
    }
  }
