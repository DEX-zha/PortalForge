...
export function reverseReferences(buf, graph, targetOffset, { conventions = ['objrel', 'abs', 'flagged', 'low24', 'secrel'] } = {}) {
  ...
  const ownerOf = p => { let lo = 0, hi = sortedOffsets.length - 1, best = -1; while (lo <= hi) { const mid = (lo + hi) >> 1; if (sortedOffsets[mid] <= p) { best = mid; lo = mid + 1; } else hi = mid - 1; } if (best < 0) return null; const o = owners.get(sortedOffsets[best]); return p < o.offset + o.size ? o : null; };
  const hits = [];
  for (const s of graph.sections) {
    ...
    for (let p = s.offset; p + 4 <= end; p += 4) {
      ...
      else if (conventions.includes('low24') && (v >>> 24) !== 0 && (v & 0xffffff) === (rel & 0xffffff) && rel < 0x1000000) conv = 'low24';
      else if (conventions.includes('secrel') && s.index !== graph.object_section && v === targetOffset - s.offset) conv = `secrel:${s.index}`;
      if (!conv) continue;
      const o = s.index === graph.object_section ? ownerOf(p) : null;
      hits.push({ file_offset: p, section: s.index, convention: conv, value: v, referrer: o ? { offset: o.offset, type_name: o.type_name, id: o.id, field: p - o.offset } : null });
    }
  }
  return hits;
}
