...
export function corpusReport(files, { fixups = {} } = {}) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      rows.push({
        file: abs,
        name: path.basename(path.dirname(path.dirname(abs))) || path.basename(abs),
        error: e.message,
      });
      continue;
    }
    ...
    for (const r of res.rows) {
      ...
      schema.invalid++;
      if (schema.errors.length < 3)
        schema.errors.push({
          offset: r.offset,
          errors: (validate.errors ?? []).slice(0, 2).map(e => `${e.instancePath} ${e.message}`),
        });
    }
    const tally = {};
    for (const s of SEVERITY) tally[s] = 0;
    ...
    const row = {
      ...
      name: path.basename(path.dirname(path.dirname(abs))) || path.basename(abs),
      has_runtime_map: !!fx,
      detected: res.file_has_placements
        ? {
      ...
      rates: {
        resolved: pct((res.counts.direct ?? 0) + (res.counts.indirect ?? 0), res.placements),
      ...
      models: {
        records: res.models.length,
        shared: res.shared_models,
        inside_a_placement: res.models_inside_a_placement,
        shared_rate: pct(res.shared_models, res.models.length),
        max_users: res.models[0]?.user_count ?? 0,
      },
      safety: tally,
      schema,
    };
    if (fx) {
      ...
      row.ground_truth = {
        total: v.total,
        agreed: v.agreed,
        agreement: pct(v.agreed, v.total),
        false_positives: v.false_positives.length,
  ...
  const totals = {
    ...
    direct: sum(r => r.counts.direct),
    indirect: sum(r => r.counts.indirect),
    ambiguous: sum(r => r.counts.ambiguous),
    absent: sum(r => r.counts.absent),
    model_records: sum(r => r.models.records),
    shared_models: sum(r => r.models.shared),
    models_inside: sum(r => r.models.inside_a_placement),
    schema_checked: sum(r => r.schema.checked),
    schema_invalid: sum(r => r.schema.invalid),
    safety: SEVERITY.reduce((a, s) => ((a[s] = sum(r => r.safety[s])), a), {}),
  };
  totals.rates = {
    resolved: pct(totals.direct + totals.indirect, totals.placements),
    ambiguous: pct(totals.ambiguous, totals.placements),
    absent: pct(totals.absent, totals.placements),
    shared_models: pct(totals.shared_models, totals.model_records),
    models_inside: pct(totals.models_inside, totals.model_records),
    schema_valid: pct(totals.schema_checked - totals.schema_invalid, totals.schema_checked),
  };
...
export function formatCorpus(rep) {
  const L = [];
  L.push(
    `placement model v1 over ${rep.totals.files} file(s): ${rep.totals.placements} placements, ${rep.totals.rates.resolved}% resolved, ${rep.totals.rates.ambiguous}% ambiguous, ${rep.totals.rates.absent}% without a model`,
  );
  L.push(
    `schema placement-v1: ${rep.totals.schema_checked - rep.totals.schema_invalid}/${rep.totals.schema_checked} records valid (${rep.totals.rates.schema_valid}%)`,
  );
  L.push(
    `model records: ${rep.totals.model_records}, ${rep.totals.shared_models} shared (${rep.totals.rates.shared_models}%), ${rep.totals.models_inside} inside another placement's blob (${rep.totals.rates.models_inside}%)`,
  );
  L.push(`safety: ` + SEVERITY.map(s => `${s} ${rep.totals.safety[s]}`).join(', '));
  L.push('');
  L.push(
    'file                      class  placements  resolved  ambig  absent  layered  models  shared  schema  safety worst-of',
  );
  for (const r of rep.rows) {
    if (r.error) {
      L.push(`${r.name.padEnd(24)} ERROR ${r.error}`);
      continue;
    }
    ...
    if (r.ground_truth)
      L.push(
        `${''.padEnd(24)} ground truth vs the runtime fixup map: ${r.ground_truth.agreed}/${r.ground_truth.total} (${r.ground_truth.agreement}%), ${r.ground_truth.false_positives} structural-only, ${r.ground_truth.missed} missed`,
      );
    for (const e of r.schema.errors)
      L.push(`${''.padEnd(24)} schema failure at 0x${e.offset.toString(16)}: ${e.errors.join('; ')}`);
  }
  return L.join('\n');
}
