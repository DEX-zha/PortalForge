...
export function levelContext(buf, graph, fixups = null) {
  ...
  const ctx = { buf, graph, sec, s2, table, spans, ptrSet: fixups ? new Set(fixups.pointer_words) : null, fixups,
    placementType: null, modelType: null, detection: null };
  return ctx;
}
...
export function signature(ctx, off) {
  ...
  const x = ctx.buf.readFloatBE(off + FIELDS.position), y = ctx.buf.readFloatBE(off + FIELDS.position + 4), z = ctx.buf.readFloatBE(off + FIELDS.position + 8);
  const named = !!(stringAt(ctx, ctx.buf.readUInt32BE(off + FIELDS.name)) ?? '').match(/[A-Za-z]{2}/);
  const scale = ctx.buf.readFloatBE(off + FIELDS.scale), heading = ctx.buf.readFloatBE(off + FIELDS.heading);
...
export function detectClasses(ctx, { minStrict = 3, minRate = 0.25 } = {}) {
  ...
  for (const e of ctx.table) { const t = ctx.buf.readUInt32BE(e); if (!byType.has(t)) byType.set(t, []); byType.get(t).push(e); }
  const candidates = [];
  for (const [type, entries] of byType) {
    let strict = 0, weak = 0; const assetTypes = new Map();
    for (const e of entries) { const s = signature(ctx, e); if (s.weak) weak++; if (s.strict) { strict++; assetTypes.set(s.assetType, (assetTypes.get(s.assetType) ?? 0) + 1); } }
    if (!strict) continue;
    const dominant = [...assetTypes.entries()].sort((a, b) => b[1] - a[1])[0];
    candidates.push({ type, label: typeLabel(ctx, type), entries: entries.length, strict, weak,
      strict_rate: strict / entries.length, weak_rate: weak / entries.length, model_type: dominant[0], model_share: dominant[1] / strict });
  }
  ...
  ctx.detection = { chosen, candidates: candidates.slice(0, 5), runner_up: candidates[1] ?? null,
...
export function resolveModel(ctx, offset) {
  ...
  if (direct) return { status: 'direct', field: FIELDS.model, model: direct, candidates: [{ ...direct, field: FIELDS.model }] };
  const found = new Map();
  ...
  if (candidates.length === 1) return { status: 'indirect', field: candidates[0].field, model: candidates[0], candidates };
...
export function ownerOf(ctx, offset) {
  let owner = null;
  for (const e of ctx.table) { if (e <= offset) owner = e; else break; }
...
export function resolveAll(buf, graph, fixups = null, { detect = true, placementType = null } = {}) {
  ...
  const users = new Map();
  for (const r of rows) if (r.model.offset !== null) { if (!users.has(r.model.offset)) users.set(r.model.offset, []); users.get(r.model.offset).push(r.offset); }
  const models = [...users.entries()].map(([offset, list]) => {
    const owner = ownerOf(ctx, offset);
    return { offset, path: assetAt(ctx, offset)?.path ?? null, users: list, user_count: list.length,
      owner: owner ? { offset: owner.offset, type: owner.type, inside_blob: owner.inside } : null,
      shared: list.length > 1, rewritten_by_replacing: owner && owner.inside ? owner.offset : null };
  }).sort((a, b) => b.user_count - a.user_count);
  const byOffset = new Map(models.map(m => [m.offset, m]));
  for (const r of rows) r.shared_state = r.model.offset !== null
    ? { model_record: r.model.offset, users: byOffset.get(r.model.offset).user_count, shared: byOffset.get(r.model.offset).shared, rewritten_by_replacing: byOffset.get(r.model.offset).rewritten_by_replacing }
    : null;
  const counts = rows.reduce((a, r) => (a[r.status] = (a[r.status] ?? 0) + 1, a), {});
  return { placements: rows.length, counts, rows, models, detection,
    ...
    classes: { placement_type: ctx.placementType, placement_label: ctx.placementType !== null ? typeLabel(ctx, ctx.placementType) : null,
      model_type: ctx.modelType, model_label: ctx.modelType !== null ? typeLabel(ctx, ctx.modelType) : null } };
}
...
export function validateAgainstFixups(buf, graph, fixups) {
  ...
  const rows = res.rows.map(r => {
    ...
    const agree = structural !== null ? (runtimePointer && runtimeTarget === structural) : !(runtimePointer && modelAt(ctx, runtimeTarget));
    return { offset: r.offset, name: r.name, status: r.status, field, agree, runtime_pointer: runtimePointer, runtime_target: runtimeTarget, structural_target: structural };
  });
  const agreed = rows.filter(r => r.agree).length;
  return { total: rows.length, agreed, agreement: rows.length ? agreed / rows.length : 1,
...
export function formatResolve(res, { limit = 20 } = {}) {
  if (!res.file_has_placements) return 'no placement class detected in this file (no header-table class carries a world position plus a resolvable model)';
  ...
  const lines = [
    ...
    `placements ${res.placements}: ` + Object.entries(res.counts).map(([k, v]) => `${k} ${v}`).join(', '),
  ...
  for (const m of res.models.slice(0, limit)) lines.push(`  0x${m.offset.toString(16)} ${String(m.path ?? '?').replace(/^.*\//, '').padEnd(36)} used by ${String(m.user_count).padStart(3)}${m.rewritten_by_replacing !== null ? `  [inside 0x${m.rewritten_by_replacing.toString(16)}]` : ''}`);
