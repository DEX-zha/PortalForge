...
export function planClone(buf, { objectOffset, findingId, edits = [], appendToList = false, findingsOpts = {} }) {
  ...
  if (!source) { const e = new Error(`No object at 0x${objectOffset.toString(16)}`); e.exitCode = 1; throw e; }
  ...
  for (const ed of edits) {
    ...
    changes.push({ field: '+0x' + ed.offset.toString(16), type: ed.type, old_hex, new_hex: cloneBytes.subarray(ed.offset, ed.offset + width).toString('hex') });
  }
  ...
  const inserted = Buffer.alloc(padded); cloneBytes.copy(inserted);
  ...
  for (const s of graph.sections) {
    ...
    if (s.index === objSec.index) { out.writeUInt32BE(s.size + padded, row + 4); updates.push({ location: row + 4, field: `sections[${s.index}].size`, old: s.size, new: s.size + padded }); }
    else if (s.offset >= insertAt) { out.writeUInt32BE(s.offset + padded, row); updates.push({ location: row, field: `sections[${s.index}].offset`, old: s.offset, new: s.offset + padded }); }
  }
  let listUpdate = null;
  if (appendToList) {
    ...
    const count = out.readUInt32BE(base + 0x0c), ptr = out.readUInt32BE(base + 0x14) & 0x7fffffff;
    ...
    let slot = -1; for (let i = 0; i < count; i++) if (out.readUInt32BE(base + ptr + 4 * i) === 0xffffffff) { slot = i; break; }
    if (slot < 0) failures.push({ stage: 'count', reason: 'no null slot in the section-1 list; growing the list would shift all objects (not implemented)', offset: arrayEnd });
    else { out.writeUInt32BE(cloneRel, base + ptr + 4 * slot); listUpdate = { location: base + ptr + 4 * slot, field: `list[${slot}]`, old: 0xffffffff, new: cloneRel }; updates.push(listUpdate); }
  }
  // Validation: re-parse.
  let after = null;
  try {
    ...
    if (!clone) failures.push({ stage: 'validation', reason: 'clone header not recognised at the insert offset', offset: insertAt });
    else {
      const resolver = makeResolver(out, { sections: after.sections }, after.sections[after.object_section], after.objects);
      ...
      if (dangling.length) failures.push({ stage: 'reference', reason: `${dangling.length} object reference(s) of the clone point past the inserted area`, offset: dangling[0].offset });
      if (after.objects.filter(o => o.id === newId).length !== 1) failures.push({ stage: 'validation', reason: 'new id not unique' });
    }
  } catch (e) { failures.push({ stage: 'validation', reason: e.message }); }
  const plan = { source: { object_offset: source.offset, type_name: source.type_name, finding_id: findingId }, changes, insert_at: insertAt, updates, new_id: newId,
    validation: { status: failures.length ? 'INVALID' : 'VALID', failures }, inserted_bytes: padded, append_to_list: appendToList, list_update: listUpdate };
