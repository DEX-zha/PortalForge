...
export function loadFixups(file) {
  ...
  for (const k of ['pointer_words', 'head_pointer_words', 'id_words', 'section_offset'])
    if (fx[k] === undefined) throw new Error(`fixup map lacks ${k}`);
  return fx;
}

const HEAD_COUNT_A = 0x0c,
  HEAD_COUNT_B = 0x10,
  HEAD_TABLE_END = 0x14,
  HEAD_BLOCK_END = 0x1c;
...
export function insertBytes(buf, graph, { pointerWords, idWords = [] }, at, bytes) {
  ...
  if (at < sec.offset + 0x20 || at > sec.offset + sec.size)
    throw new Error(`insert point 0x${at.toString(16)} outside the object section`);
  ...
  const rebase = (wordOffset, field) => {
    if (done.has(wordOffset)) return;
    done.add(wordOffset);
    const w = move(wordOffset);
    const v = out.readUInt32BE(w);
    const flag = v & 0x80000000,
      rel = v & 0x7fffffff;
    if (rel >= relAt) {
      out.writeUInt32BE((flag | (rel + len)) >>> 0, w);
      updates.push({ location: w, field, old: v, new: (flag | (rel + len)) >>> 0 });
    }
  };
  ...
  if (at <= tableEnd) {
    for (const h of [HEAD_COUNT_A, HEAD_COUNT_B]) {
      const v = out.readUInt32BE(sec.offset + h);
      out.writeUInt32BE(v + len / 4, sec.offset + h);
      updates.push({ location: sec.offset + h, field: `head+0x${h.toString(16)} count`, old: v, new: v + len / 4 });
    }
  ...
  for (const s of graph.sections) {
    ...
    if (s.index === sec.index) {
      out.writeUInt32BE(s.size + len, row + 4);
      updates.push({ location: row + 4, field: `sections[${s.index}].size`, old: s.size, new: s.size + len });
    } else if (s.offset >= at) {
      out.writeUInt32BE(s.offset + len, row);
      updates.push({ location: row, field: `sections[${s.index}].offset`, old: s.offset, new: s.offset + len });
    }
  }
...
export function planReachableClone(
  buf,
  fixups,
  {
    start,
  ...
  if (fixups.section_offset !== sec.offset)
    throw new Error(
      `fixup map is for a file whose object section starts at 0x${fixups.section_offset.toString(16)}, this one starts at 0x${sec.offset.toString(16)}`,
    );
  const source = graph.objects.find(o => o.offset === start);
  if (!source) {
    const e = new Error(`No object header at 0x${start.toString(16)}`);
    e.exitCode = 1;
    throw e;
  }
  ...
  if (insertAt !== secEnd && !graph.objects.some(o => o.offset === insertAt))
    throw new Error(`insertBefore 0x${insertAt.toString(16)} is not an object header`);
  ...
  const copy = Buffer.alloc(prePad + blockLen + (inside ? 0 : pad));
  buf.copy(copy, prePad, start, end);
  ...
  for (const w of blockPointerWords) {
    ...
    const rel = v & 0x7fffffff,
      target = sec.offset + rel;
    if (target >= start && target < end) {
      copy.writeUInt32BE(((v & 0x80000000) | (rel + delta)) >>> 0, at(w));
      internal++;
    } else {
      external++;
      externalTargets.set(target, (externalTargets.get(target) ?? 0) + 1);
      if (target >= insertAt) copy.writeUInt32BE(((v & 0x80000000) | (rel + copy.length)) >>> 0, at(w));
    }
  }
  const work = buf;
  ...
  const newIds = [];
  if (freshIds)
    for (const w of idWordsInBlock) {
      const old = copy.readUInt32BE(at(w));
      copy.writeUInt32BE(nextId >>> 0, at(w));
      newIds.push({
        field: '+0x' + (w - start).toString(16),
        old: '0x' + old.toString(16),
        new: '0x' + nextId.toString(16),
      });
      nextId++;
    }
  ...
  const words = {
    pointerWords: fixups.pointer_words.concat(fixups.head_pointer_words, crossWords),
  ...
  let out = step.buffer,
    tableEntry = null,
    cloneOffset = cloneStart;
  if (endPad) {
    const g1 = buildGraph(out, { fields: false });
    const s1 = g1.sections[g1.object_section];
    step = insertBytes(
      out,
      g1,
      { pointerWords: pointerWordsNow, idWords: step.idWords },
      s1.offset + s1.size,
      Buffer.alloc(endPad),
    );
    out = step.buffer;
    ...
    updates.push({
      location: s1.offset + s1.size,
      field: 'alignment padding at the section end',
      old: null,
      new: endPad,
    });
  }
  ...
  if (register) {
    ...
    entry.writeUInt32BE(cloneOffset + shift - sec1.offset); // the clone itself moves by `shift`
    ...
    tableEntry = {
      location: tableEnd,
      value: cloneOffset + shift - 4 - sec1.offset,
      index: (tableEnd - sec1.offset) / 4,
    };
  ...
  if (replaceEntry !== null) {
    ...
    const tableEnd = sec.offset + (buf.readUInt32BE(sec.offset + HEAD_TABLE_END) & 0x7fffffff);
    if (loc < sec.offset + 0x20 || loc >= tableEnd)
      throw new Error(`table index ${replaceEntry} outside the header table`);
    const old = out.readUInt32BE(loc);
    out.writeUInt32BE(cloneOffset - sec.offset, loc);
    tableEntry = {
      location: loc,
      value: cloneOffset - sec.offset,
      index: replaceEntry,
      replaced: old,
      replaced_object: sec.offset + old,
    };
    updates.push({
      location: loc,
      field: `head table entry #${replaceEntry} (replaced)`,
      old,
      new: cloneOffset - sec.offset,
    });
  }
  ...
  try {
    ...
    if (after.objects.length !== expected)
      failures.push({ stage: 'count', reason: `object count ${after.objects.length}, expected ${expected}` });
    ...
    if (
      secA.offset + secA.size !==
      after.sections[after.sections.length - 1].offset -
        (after.sections[after.sections.length - 1].offset - (secA.offset + secA.size))
    ) {
      /* sections are contiguous by construction */
    }
    if (after.sections.at(-1).offset + after.sections.at(-1).size !== out.length)
      failures.push({ stage: 'validation', reason: 'sections do not end at EOF' });
    ...
    else if (clone.type !== source.type)
      failures.push({ stage: 'validation', reason: `clone type ${clone.type} != source ${source.type}` });
    ...
    for (const o of graph.objects) {
      const m = byOff.get(moved(o));
      if (!m || m.type !== o.type) mismatched++;
    }
    if (mismatched)
      failures.push({
        stage: 'validation',
        reason: `${mismatched} original object(s) not found at their shifted offset`,
      });
    // pointer words of the copy resolve to the same object types as in the source
    for (const w of blockPointerWords) {
      const vs = buf.readUInt32BE(w) & 0x7fffffff,
        vc = out.readUInt32BE(w - start + cloneOffset) & 0x7fffffff;
      const ts = sec.offset + vs,
        tc = after.sections[after.object_section].offset + vc;
      const srcObj = ownerOf(graph.objects, ts),
        cloneObj = ownerOf(after.objects, tc);
      if (!srcObj || !cloneObj || srcObj.type !== cloneObj.type || ts - srcObj.offset !== tc - cloneObj.offset) {
        failures.push({
          stage: 'reference',
          reason: `copied pointer +0x${(w - start).toString(16)} does not resolve like the original`,
          offset: w - start + cloneOffset,
        });
        break;
      }
    }
    if (register) {
      ...
      const cnt = out.readUInt32BE(secA2.offset + HEAD_COUNT_A),
        endRel = out.readUInt32BE(secA2.offset + HEAD_TABLE_END) & 0x7fffffff;
      if (cnt * 4 !== endRel)
        failures.push({ stage: 'validation', reason: `table count ${cnt} and end 0x${endRel.toString(16)} disagree` });
      if (secA2.offset + out.readUInt32BE(secA2.offset + HEAD_BLOCK_END) !== after.objects[0].offset)
        failures.push({ stage: 'validation', reason: 'block end no longer points at the first object' });
      if (out.readUInt32BE(tableEntry.location) !== cloneOffset - secA2.offset)
        failures.push({ stage: 'reference', reason: 'table entry does not point at the clone' });
    }
  } catch (e) {
  ...
  if (bumpRefcounts) {
    ...
    for (const [t, n] of externalTargets) {
      ...
      if (rc >= 1 && rc <= 100000) {
        out.writeUInt32BE(rc + n, loc);
        refcountUpdates.push({ location: loc, field: 'refcount', old: rc, new: rc + n });
      } else
        failures.push({
          stage: 'reference',
          reason: `shared target 0x${t.toString(16)} has no plausible refcount (${rc})`,
          offset: t,
        });
    }
  ...
  const plan = {
    source: {
      ...
      block_end: end,
      block_bytes: blockLen,
    },
    changes,
    insert_at: cloneOffset,
    updates: updates.slice(0, 200),
    updates_total: updates.length,
    new_id: freshIds ? maxId + 1 : source.id,
    new_ids: newIds,
    fresh_ids: freshIds,
    ...
    insert_before: inside ? insertAt : null,
    pre_pad: prePad,
    end_pad: endPad,
    refcounts: refcountUpdates,
    table_entry: tableEntry,
    pointers: {
      internal,
      external,
      cross_section_words: crossWords.length,
      rebased_after_shift: updates.filter(u => u.field === 'pointer').length,
    },
...
function ownerOf(objects, fileOffset) {
  ...
  while (lo <= hi) {
    const m = (lo + hi) >> 1;
    if (objects[m].offset <= fileOffset) {
      best = m;
      lo = m + 1;
    } else hi = m - 1;
  }
  ...
  return fileOffset < o.offset + o.size ? o : null;
}
...
export function planOverwriteClone(
  buf,
  fixups,
  ...
  if (fixups.section_offset !== sec.offset)
    throw new Error(
      `fixup map is for section start 0x${fixups.section_offset.toString(16)}, this file 0x${sec.offset.toString(16)}`,
    );
  const source = graph.objects.find(o => o.offset === start);
  ...
  for (let p = sec.offset + 0x20; p < tableEnd; p += 4) entries.add(sec.offset + buf.readUInt32BE(p));
  if (!entries.has(target)) failures.push({ stage: 'validation', reason: 'target is not a header-table entry' });
  if (tgtObj.type !== source.type)
    failures.push({ stage: 'validation', reason: `target type ${tgtObj.type} != clone owner type ${source.type}` });
  let blobEnd = sec.offset + sec.size;
  for (const e of entries) if (e > target && e < blobEnd) blobEnd = e;
  if (blobEnd - target < blockLen)
    throw Object.assign(
      new Error(`target blob 0x${(blobEnd - target).toString(16)} < block 0x${blockLen.toString(16)}`),
      { exitCode: 1 },
    );
  ...
  let internal = 0,
    external = 0;
  const cloneExternals = new Map();
  for (const w of fixups.pointer_words.filter(inBlock)) {
    const off = w - start + target,
      v = out.readUInt32BE(off),
      rel = v & 0x7fffffff,
      tgt = sec.offset + rel;
    if (tgt >= start && tgt < end) {
      out.writeUInt32BE(((v & 0x80000000) | (rel + delta)) >>> 0, off);
      internal++;
    } else {
      external++;
      cloneExternals.set(tgt, (cloneExternals.get(tgt) ?? 0) + 1);
    }
  }
  ...
  const refcountUpdates = [];
  if (bumpRefcounts)
    for (const [t, n] of cloneExternals) {
      if (t >= target && t < blobEnd) continue; // inside the overwritten blob
      const rc = out.readUInt32BE(t + 4);
      if (rc >= 1 && rc <= 200000) {
        out.writeUInt32BE(rc + n, t + 4);
        refcountUpdates.push({ location: t + 4, field: 'refcount', old: rc, new: rc + n });
      } else
        failures.push({
          stage: 'reference',
          reason: `shared target 0x${t.toString(16)} refcount ${rc} implausible`,
          offset: t,
        });
    }
  ...
  for (let p = 0; p + 4 <= buf.length; p += 4) {
    if (p >= target && p < blobEnd) continue;
    if (rcLocs.has(p)) continue;
    if (buf.readUInt32BE(p) !== out.readUInt32BE(p)) {
      failures.push({ stage: 'validation', reason: `byte change outside the target blob at 0x${p.toString(16)}` });
      break;
    }
  }
  let after = null;
  try {
    ...
    if (!clone || clone.type !== source.type)
      failures.push({ stage: 'validation', reason: 'clone owner not recognised at the target', offset: target });
    ...
    for (const rel of childRels) {
      const child = after.objects.find(o => o.offset === target + rel);
      if (!child)
        failures.push({
          stage: 'validation',
          reason: `clone child at +0x${rel.toString(16)} not recognised`,
          offset: target + rel,
        });
    }
    if (after.issues.length)
  ...
  const plan = {
    ...
    changes,
    insert_at: target,
    updates: refcountUpdates,
    new_id: source.id,
    validation: { status: failures.length ? 'INVALID' : 'VALID', failures },
    mode: 'overwrite',
    target: { offset: target, type_name: tgtObj.type_name, blob_bytes: blobEnd - target, leftover_zeroed: leftover },
    pointers: { internal, external },
    refcounts: refcountUpdates,
    findings: [findingId, ...extraFindings],
    method:
...
export function planLinkClone(
  ...
  for (let q = 12; q < P.size; q += 4)
    if (q !== linkField && ptrSet.has(P.offset + q))
      failures.push({
        stage: 'reference',
        reason: 'source has another section-1 pointer at +0x' + q.toString(16) + '; not a pure chain node',
        offset: P.offset + q,
      });
  const oldNextRel = buf.readUInt32BE(P.offset + linkField);
  const insertAt = insertBefore ?? sec.offset + sec.size;
  if (insertAt !== sec.offset + sec.size && !graph.objects.some(o => o.offset === insertAt))
    throw new Error('insertBefore 0x' + insertAt.toString(16) + ' is not an object header');
  ...
  const newNextRel =
    oldNextRel !== 0 && sec.offset + (oldNextRel & 0xffffff) >= insertAt ? oldNextRel + P.size : oldNextRel;
  const copy = Buffer.alloc(P.size);
  ...
  try {
    ...
    if (!c || c.type !== P.type)
      failures.push({ stage: 'validation', reason: 'clone not recognised at the insert point', offset: cloneOffset });
    ...
    if (after.sections.at(-1).offset + after.sections.at(-1).size !== out.length)
      failures.push({ stage: 'validation', reason: 'sections do not end at EOF' });
    const secA = after.sections[after.object_section];
    if (secA.offset + out.readUInt32BE(pLink) !== cloneOffset)
      failures.push({ stage: 'reference', reason: 'source.next does not point at the clone' });
    if (out.readUInt32BE(cloneOffset + linkField) !== newNextRel >>> 0)
      failures.push({ stage: 'reference', reason: 'clone.next differs from the (rebased) old next' });
    if (newNextRel !== 0 && after.sections[after.object_section].offset + (newNextRel & 0xffffff) === cloneOffset)
      failures.push({ stage: 'reference', reason: 'clone.next points at the clone itself (self-loop)' });
    for (const o of graph.objects.filter(o => o.offset >= insertAt)) {
      const m = after.objects.find(x => x.offset === o.offset + copy.length);
      if (!m || m.type !== o.type) {
        failures.push({
          stage: 'validation',
          reason: 'moved record 0x' + o.offset.toString(16) + ' not found at its new offset',
        });
        break;
      }
    }
  ...
  const plan = {
    source: {
      object_offset: P.offset,
      type_name: P.type_name,
      finding_id: findingId,
      block_end: P.offset + P.size,
      block_bytes: P.size,
    },
    changes,
    insert_at: cloneOffset,
    ...
    old_next: oldNextRel,
    new_next: newNextRel,
    moved_records: graph.objects.filter(o => o.offset >= insertAt).length,
    inserted_bytes: copy.length + pad,
    refcounts: [],
    findings: [findingId, ...extraFindings],
...
export function planReplaceNode(
  buf,
  fixups,
  ...
  const Vo = graph.objects.find(o => o.offset === V);
  if (!Vo) throw Object.assign(new Error('victim 0x' + V.toString(16) + ' is not an object header'), { exitCode: 1 });
  const failures = [],
    changes = [];
  if (Vo.type !== P.type)
    failures.push({ stage: 'validation', reason: 'victim type ' + Vo.type + ' != source type ' + P.type });
  if (Vo.size !== P.size)
    failures.push({
      stage: 'validation',
      reason: 'victim size 0x' + Vo.size.toString(16) + ' != source size 0x' + P.size.toString(16),
    });
  const ptrSet = new Set(fixups.pointer_words);
  for (const o of [P, Vo])
    for (let q = 12; q < o.size; q += 4)
      if (q !== linkField && ptrSet.has(o.offset + q))
        failures.push({
          stage: 'reference',
          reason: (o === P ? 'source' : 'victim') + ' has another section-1 pointer at +0x' + q.toString(16),
          offset: o.offset + q,
        });
  ...
  for (let p = 0; p + 4 <= buf.length; p += 4) {
    ...
    if (buf.readUInt32BE(p) !== out.readUInt32BE(p)) {
      failures.push({ stage: 'validation', reason: 'byte change outside the victim at 0x' + p.toString(16) });
      break;
    }
  }
  let after = null;
  try {
    after = buildGraph(out, { fields: false });
    const c = after.objects.find(o => o.offset === V);
    if (!c || c.type !== P.type)
      failures.push({ stage: 'validation', reason: 'copy not recognised at the victim slot', offset: V });
    if (sec.offset + (out.readUInt32BE(P.offset + linkField) & 0xffffff) !== V)
      failures.push({ stage: 'reference', reason: 'source.next does not point at the victim slot' });
    if (out.readUInt32BE(V + linkField) !== vNext)
      failures.push({ stage: 'reference', reason: 'copy.next lost the victim old next' });
    if (after.issues.length)
      failures.push(...after.issues.map(i => ({ stage: 'validation', reason: i.reason, offset: i.offset })));
  } catch (e) {
    failures.push({ stage: 'validation', reason: e.message });
  ...
  const plan = {
    source: { object_offset: P.offset, type_name: P.type_name, finding_id: findingId },
    changes,
    insert_at: V,
    updates: [
      { location: V + 4, field: 'refcount', old: buf.readUInt32BE(V + 4), new: 1 },
      { location: V + linkField, field: 'copy.next (victim old next kept)', old: vNext, new: vNext },
    ],
    new_id: P.id,
    validation: { status: failures.length ? 'INVALID' : 'VALID', failures },
    mode: 'replace-node',
    victim: V,
    victim_type_name: Vo.type_name,
    victim_old_next: vNext,
    link_field: linkField,
    findings: [findingId, ...extraFindings],
...
export function planReplaceRecord(
  buf,
  fixups,
  {
    source,
  ...
  const S = resolve ? resolve(buf, source) : graph.objects.find(o => o.offset === source),
    V = resolve ? resolve(buf, victim) : graph.objects.find(o => o.offset === victim);
  if (!S || !V) throw Object.assign(new Error('source or victim is not an object header'), { exitCode: 1 });
  const failures = [],
    changes = [];
  if (S.type !== V.type)
    failures.push({ stage: 'validation', reason: 'victim type ' + V.type + ' != source type ' + S.type });
  if (S.size !== V.size)
    failures.push({
      stage: 'validation',
      reason: 'victim size 0x' + V.size.toString(16) + ' != source size 0x' + S.size.toString(16),
    });
  const ptrSet = new Set(fixups.pointer_words);
  const out = Buffer.from(buf);
  buf.copy(out, V.offset, S.offset, S.offset + S.size);
  out.writeUInt32BE(refcount === 'victim' ? buf.readUInt32BE(V.offset + 4) : 1, V.offset + 4);
  const delta = V.offset - S.offset;
  let internal = 0,
    external = 0,
    kept = 0;
  for (let q = 8; q + 4 <= S.size; q += 4) {
    // word 2 (+8, the name) may be kept; words 0/1 are type and refcount
    if (keep.includes(q)) {
      out.writeUInt32BE(buf.readUInt32BE(V.offset + q), V.offset + q);
      kept++;
      continue;
    }
    if (q < 12) continue;
    if (!ptrSet.has(S.offset + q)) continue;
    const v = buf.readUInt32BE(S.offset + q);
    const t = sec.offset + (v & 0x7fffffff);
    if (t >= S.offset && t < S.offset + S.size) {
      out.writeUInt32BE(((v & 0x80000000) | ((v & 0x7fffffff) + delta)) >>> 0, V.offset + q);
      internal++;
    } else {
      external++;
      const rc = out.readUInt32BE(t + 4);
      if (rc >= 1 && rc <= 200000) out.writeUInt32BE(rc + 1, t + 4);
    }
  }
  ...
  for (const w of fixups.pointer_words) {
    ...
    const entry = {
      from: w,
      delta: d,
      victim_table_entry: tableSet.has(t),
      source_table_entry: tableSet.has(S.offset + d),
    };
    if (entry.victim_table_entry) {
      entry.victim_type = buf.readUInt32BE(t);
      entry.source_type = buf.readUInt32BE(S.offset + d);
      if (!entry.source_table_entry)
        failures.push({
          stage: 'validation',
          reason:
            'a pointer from 0x' +
            w.toString(16) +
            ' addresses the header-table record at victim+0x' +
            d.toString(16) +
            ', but the source has no header-table record at that offset',
        });
      else if (entry.victim_type !== entry.source_type)
        failures.push({
          stage: 'validation',
          reason:
            'the header-table record at victim+0x' +
            d.toString(16) +
            ' is type ' +
            entry.victim_type +
            ' but the source has type ' +
            entry.source_type +
            ' there',
        });
    }
  ...
  const readName = (b, p) => {
    const v = b.readUInt32BE(p + 8);
    if (v >>> 24 !== 1) return null;
    const off = v & 0xffffff;
    if (off >= s2.size) return null;
    const q = s2.offset + off;
    const e2 = b.indexOf(0, q);
    return b.toString('latin1', q, e2 < 0 ? q + 60 : Math.min(e2, q + 200));
  };
  const deltas = [...new Set(midblob.map(e => e.delta))].sort((a, b2) => a - b2);
  const shared = [];
  for (let k = 0; k < deltas.length; k++) {
    const d = deltas[k];
    const stop = deltas[k + 1] ?? V.size;
    const users = midblob.filter(e => e.delta === d).length;
    let touched = false;
    for (let q = d; q < stop && q + 4 <= V.size; q += 4)
      if (buf.readUInt32BE(V.offset + q) !== out.readUInt32BE(V.offset + q)) {
        touched = true;
        break;
      }
    const before = readName(buf, V.offset + d),
      afterName = readName(out, V.offset + d);
    shared.push({
      delta: d,
      offset: V.offset + d,
      external_users: users,
      bytes_changed: touched,
      name_before: before,
      name_after: afterName,
    });
  }
  ...
  if (warn.length)
    warnings.push(
      ...warn.map(
        x =>
          'the record at victim+0x' +
          x.delta.toString(16) +
          ' is referenced by ' +
          x.external_users +
          ' record(s) outside the victim and the copy changes its bytes' +
          (x.name_before !== x.name_after ? ' (name "' + x.name_before + '" -> "' + x.name_after + '")' : '') +
          ': every user of that shared record is affected',
      ),
    );
  if (out.length !== buf.length) failures.push({ stage: 'validation', reason: 'file length changed' });
  const rcLocs = new Set();
  for (let q = 12; q + 4 <= S.size; q += 4)
    if (!keep.includes(q) && ptrSet.has(S.offset + q)) {
      const t = sec.offset + (buf.readUInt32BE(S.offset + q) & 0x7fffffff);
      if (!(t >= S.offset && t < S.offset + S.size)) rcLocs.add(t + 4);
    }
  for (let p = 0; p + 4 <= buf.length; p += 4) {
    if (p >= V.offset && p < V.offset + V.size) continue;
    if (rcLocs.has(p)) continue;
    if (buf.readUInt32BE(p) !== out.readUInt32BE(p)) {
      failures.push({ stage: 'validation', reason: 'byte change outside the victim at 0x' + p.toString(16) });
      break;
  ...
  try {
    after = buildGraph(out, { fields: false });
    const c = resolve ? resolve(out, V.offset) : after.objects.find(o => o.offset === V.offset);
    if (!c || c.type !== S.type)
      failures.push({ stage: 'validation', reason: 'copy not recognised at the victim slot' });
    if (after.issues.length)
      failures.push(...after.issues.map(i => ({ stage: 'validation', reason: i.reason, offset: i.offset })));
  } catch (e) {
    failures.push({ stage: 'validation', reason: e.message });
  ...
  const plan = {
    source: { object_offset: S.offset, type_name: S.type_name, finding_id: findingId },
    changes,
    insert_at: V.offset,
    updates: [
      {
        location: V.offset + 4,
        field: 'refcount',
        old: buf.readUInt32BE(V.offset + 4),
        new: out.readUInt32BE(V.offset + 4),
      },
    ],
    new_id: S.id,
    validation: { status: failures.length ? 'INVALID' : 'VALID', failures },
    mode: 'replace-record',
    victim: V.offset,
    victim_type_name: V.type_name,
    kept_fields: keep.map(q => '+0x' + q.toString(16)),
    pointers: { internal, external, kept },
    refcounts: [...rcLocs].map(l => ({ location: l, field: 'refcount +1' })),
    findings: [findingId, ...extraFindings],
