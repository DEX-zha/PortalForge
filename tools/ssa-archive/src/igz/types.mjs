...
export function parseTypes(buf, header) {
  ...
  if (!s0 || !header.second_header) return { names: [], sizes: [], names_offset: null, sizes_offset: null, issues: [{ offset: 0, field: 'types', actual: 'missing section 0', expected: 'section 0 with second header', reason: 'TYPE_TABLE_MISSING' }] };
  const start = s0.offset + 0x34, limit = s0.offset + s0.size;
  ...
  const sizesStart = q => { if (q % 4 !== 0 || q + 16 > limit) return false; const n = Math.min(8, Math.floor((limit - q) / 4)); if (n < 4) return false; let nonZero = false; for (let i = 0; i < n; i++) { const w = buf.readUInt32BE(q + 4 * i); if (w >= 0x100000) return false; if (w) nonZero = true; } return nonZero; };
  void isLetter;
  while (p < limit) {
  ...
  while (names.length && names[names.length - 1] === '' && p > start && (sizesOffset - p) < 4) { break; }
  const sizes = [];
  for (let i = 0; i < names.length && sizesOffset + 4 * i + 4 <= limit; i++) sizes.push(buf.readUInt32BE(sizesOffset + 4 * i));
  return { names, sizes, names_offset: start, sizes_offset: sizesOffset, issues: [] };
}

export function typeRows(types) {
  const printable = /^[\x20-\x7e]*$/;
  return types.names.map((name, index) => ({ index, name: printable.test(name) ? name : '', size_hint: types.sizes[index] ?? null, size_confidence: 'UNKNOWN', block_record: !printable.test(name) || name === '' }));
}
