...
function stringAt(buf, graph, v) {
  const s2 = graph.sections[2]; const off = v & 0xffffff; if ((v >>> 24) !== 1 || off >= s2.size) return null;
...
const looseHeader = (buf, p) => p + 12 <= buf.length && buf.readUInt32BE(p) < 224 && buf.readUInt32BE(p + 4) >= 1 && buf.readUInt32BE(p + 4) <= 64 && (buf.readUInt32BE(p + 8) >>> 24) === 1;

export function decodeScript(buf, graph, fixups, recOffset) {
  const sec = graph.sections[graph.object_section];
  const sorted = scriptTable(buf, graph);
  if (!sorted.includes(recOffset)) throw Object.assign(new Error(`0x${recOffset.toString(16)} is not a header-table record`), { exitCode: 1 });
  ...
  const count = buf.readUInt32BE(recOffset + 0x24), cap = buf.readUInt32BE(recOffset + 0x28), sizeWord = buf.readUInt32BE(recOffset + 0x2c), arrPtr = buf.readUInt32BE(recOffset + 0x30);
  ...
  if (sizeWord !== ((0x80000000 | (count * 4)) >>> 0)) issues.push(`size word 0x${sizeWord.toString(16)} != 0x80000000|count*4`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (rec) {
      ...
      for (let q = 12; q + 4 <= rec.size; q += 4) {
        ...
        const s = stringAt(buf, graph, a); if (s !== null && /[A-Za-z0-9_]{2}/.test(s)) { args.strings.push({ at: q, text: s.slice(0, 60) }); continue; }
        if (ptrSet.has(rec.offset + q)) { const t = sec.offset + (a & 0xffffff); const to = byOff.get(t); args.refs.push({ at: q, target: t, kind: to ? (to.offset >= recOffset && to.offset < end ? 'instruction' : 'external') : 'inside', opcode: to ? stringAt(buf, graph, buf.readUInt32BE(to.offset + 8))?.slice(0, 30) : null }); continue; }
        const f = buf.readFloatBE(rec.offset + q);
        if (Number.isFinite(f) && Math.abs(f) > 1e-3 && Math.abs(f) < 1e5 && (a >>> 24) >= 0x38 && (a >>> 24) <= 0xc8) args.floats.push({ at: q, value: Math.round(f * 1000) / 1000 });
      }
      for (let k = 0; k + 2 < args.floats.length; k++) if (args.floats[k + 1].at === args.floats[k].at + 4 && args.floats[k + 2].at === args.floats[k].at + 8) { args.triples.push({ at: args.floats[k].at, xyz: [args.floats[k].value, args.floats[k + 1].value, args.floats[k + 2].value] }); k += 2; }
      entry.args = args;
    }
  ...
  return { record: recOffset, end, blob_bytes: end - recOffset, name: stringAt(buf, graph, buf.readUInt32BE(recOffset + 8)), owner_size: owner?.size ?? null, count, instructions, issues,
    coverage: { records_in_blob: inside.length, covered_bytes: covered, uncovered_bytes: end - recOffset - covered }, all_targets_in_blob: instructions.every(e => e.in_blob), all_headers: instructions.every(e => e.header), local_headers: instructions.filter(e => e.in_blob).every(e => e.header), local_count: instructions.filter(e => e.in_blob).length, shared_count: instructions.filter(e => !e.in_blob).length };
}

export function auditScripts(buf, graph, fixups) {
  const sorted = scriptTable(buf, graph);
  const scripts = sorted.filter(e => buf.readUInt32BE(e) === 92);
  const rows = scripts.map(e => { try { const d = decodeScript(buf, graph, fixups, e); return { record: e, name: d.name?.replace(/^.*\//, ''), count: d.count, issues: d.issues.length, targets_in_blob: d.all_targets_in_blob, headers: d.all_headers, uncovered: d.coverage.uncovered_bytes }; } catch (err) { return { record: e, error: err.message }; } });
