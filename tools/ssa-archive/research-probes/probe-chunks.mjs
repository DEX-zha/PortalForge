...
for (const e of p.entries)
  console.log(
    `  entry ${e.index} ${e.name} start=0x${e.start.toString(16)} size=${e.size} stored=${e.stored_size} mode=0x${e.mode.toString(16)} chunks=${Math.ceil(e.size / 0x8000)}`,
  );

function validProps(props) {
  return props[0] === 0x5d && props.readUInt32LE(1) <= 0x8000 && props.readUInt32LE(1) >= 0x1000;
}
...
function decodeCompressedChunk(abs, want) {
  const csize = buf.readUInt16BE(abs),
    props = buf.subarray(abs + 2, abs + 7);
...
for (const e of p.entries) {
  ...
  if (nChunks === 1) {
    ...
    notes.push(
      `table idx ${e.chunk_table_index}: ${tbl
        .slice(0, 4)
    ...
    for (let i = 0; i < nChunks; i++) {
      const v = tbl[i];
      const abs = e.start + (v & 0x7fff) * 0x800;
      const want = Math.min(0x8000, remaining);
      if (v & 0x8000) {
        const r = decodeCompressedChunk(abs, want);
        if (r.error) {
          notes.push(`chunk ${i} @0x${abs.toString(16)}: ${r.error}`);
          break;
        }
        parts.push(r.data);
        ...
        if (nextExpected !== nextActual)
          notes.push(
            `chunk ${i}: next expected 0x${nextExpected.toString(16)} but table says 0x${nextActual.toString(16)}`,
          );
      } else {
        parts.push(buf.subarray(abs, abs + want));
        remaining -= want;
        const nextActual = e.start + (tbl[i + 1] & 0x7fff) * 0x800;
        if (abs + want !== nextActual && i < nChunks - 1)
          notes.push(`raw chunk ${i}: next expected 0x${(abs + want).toString(16)} table 0x${nextActual.toString(16)}`);
      }
    }
  ...
  console.log(
    `${e.name}: decoded ${data.length}/${e.size} ${data.length === e.size ? 'OK' : 'INCOMPLETE'} head=${head.toString('hex')} "${head.toString('latin1').replace(/[^\x20-\x7e]/g, '.')}"`,
  );
  for (const n of notes) console.log('    ' + n);
  if (outDir && data.length === e.size) {
    fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, e.name), data);
  }
}
console.log(
  'trailing values:',
