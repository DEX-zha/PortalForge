...
console.log(
  ...
  sizes
    ...
    .join(' '),
  '… value at index names.length-1:',
  '0x' + (sizes[names.length - 1] ?? 0).toString(16),
  'next:',
  '0x' + (sizes[names.length] ?? 0).toString(16),
  '0x' + (sizes[names.length + 1] ?? 0).toString(16),
);
function dump(start, len, label) {
  console.log('--- ' + label + ' @0x' + start.toString(16));
  for (let p = start; p < start + len; p += 16) {
    const hex = [...b.subarray(p, p + 16)].map(x => x.toString(16).padStart(2, '0')).join(' ');
    const f = [0, 4, 8, 12]
      .map(i => b.readFloatBE(p + i))
      .map(v => ((Math.abs(v) < 1e6 && Math.abs(v) > 1e-4) || v === 0 ? v.toFixed(2) : '-'));
    console.log(p.toString(16).padStart(8, '0') + ': ' + hex + '   f32be: ' + f.join(' '));
  }
}
...
let n = 0,
  p = 0x4b1a0;
while (u32(p) === 0xd8418 + 0x40 * n) {
  n++;
  p += 4;
  if (n > 5000) break;
}
console.log(
  'consecutive 0x40-spaced pointers from 0x4b1a0:',
  n,
  'next word:',
  '0x' + u32(p).toString(16),
  'at 0x' + p.toString(16),
);
