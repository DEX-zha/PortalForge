...
const str = p => {
  ...
  return e > p && e - p < 80 ? b.toString('latin1', p, e) : null;
};
function describe(ob, size = 0x1a0) {
  ...
  for (let q = 0; q < size; q += 4) {
    ...
    if (!note && Number.isFinite(f) && Math.abs(f) > 1e-3 && Math.abs(f) < 1e5 && Math.abs(f) !== 1)
      note = f.toFixed(3);
...
let withMatrix = 0,
  sample = [];
for (const ob of plc) {
  for (let q = 0; q + 64 <= 0x1a0; q += 4) {
    const m = [];
    for (let i = 0; i < 16; i++) m.push(f32(ob.offset + q + 4 * i));
    if (
      m.every(v => Number.isFinite(v) && Math.abs(v) < 1e5) &&
      m[3] === 0 &&
      m[7] === 0 &&
      m[11] === 0 &&
      m[15] === 1 &&
      ...
      if (sample.length < 8)
        sample.push(
          `@0x${ob.offset.toString(16)}+0x${q.toString(16)} T=[${m[12].toFixed(1)}, ${m[13].toFixed(1)}, ${m[14].toFixed(1)}] diag=[${m[0].toFixed(2)},${m[5].toFixed(2)},${m[10].toFixed(2)}]`,
        );
      break;
    }
  }
}
console.log('placements with an embedded row-major matrix (last row 0 0 0 1):', withMatrix);
...
for (const ob of plc) {
  for (let q = 0; q + 64 <= 0x1a0; q += 4) {
    ...
    if (
      ...
      if (s2.length < 5)
        s2.push(
          `@0x${ob.offset.toString(16)}+0x${q.toString(16)} T=[${m[3].toFixed(1)}, ${m[7].toFixed(1)}, ${m[11].toFixed(1)}]`,
        );
      break;
    }
  }
}
console.log('placements with column-major matrix:', colMajor);
