...
for (const r of rows) {
  ...
  else bad.push({ path: r.path, count: r.count, w10: r.words[4].toString(16), expect: expect.toString(16) });
}
...
for (const rel of [
  ...
  'level/Challenge_Level_000.bld',
  'level/Challenge_Level_001.bld',
]) {
  const p = parseArchive(fs.readFileSync(path.join(root, rel)));
  const n = p.header.count,
    w10 = p.header.flags_packed >>> 0,
  ...
  p.hashes.forEach((h, i) => {
    ...
    maxBack = Math.max(maxBack, -d);
    maxAbs = Math.max(maxAbs, Math.abs(d));
  });
  const buckets = new Map();
  for (const e of est) buckets.set(e, (buckets.get(e) ?? 0) + 1);
  const maxBucket = Math.max(...buckets.values());
  console.log(
    rel.padEnd(34),
    `count=${n} w14=${w14} maxFwd=${maxFwd} maxBack=${maxBack} maxAbs=${maxAbs} maxBucket=${maxBucket}`,
  );
}
