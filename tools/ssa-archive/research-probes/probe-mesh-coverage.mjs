...
function tally(units) {
  ...
  for (const u of units) {
    vertices += u.count;
    triangles += u.triangles.length / 3;
...
for (const input of inputs) {
  ...
  const graph = buildGraph(buffer),
    resolved = resolveAll(buffer, graph, fixups);
  ...
  const served = modelMeshes(session),
    payload = meshesPayload(session);
  ...
  const report = {
    ...
    sha256,
    fixups: fixupFile,
    placements: resolved.rows.length,
    models: models.length,
    models_with_mesh: served.models.size,
    decode: { descriptors: geo.descriptors.length, stop: geo.stop, consumed: geo.consumed, size: geo.size },
    decoded: tally(geo.units),
    assigned_unique: tally(geo.units.filter((u, i) => assigned.has(i))),
    unassigned: tally(unassigned),
    scenery: tally(unassigned.filter(u => sceneryIds.has(u.descriptor))),
    omitted: tally(omitted),
    omitted_interleaved: tally(omitted.filter(u => u.kind === 'interleaved')),
    model_unit_associations: served.stats.owned,
    shared_units: assignment.shared,
    ...
    omitted_examples: examples.slice(0, 12).map(u => {
      ...
      return {
        ...
        owner_bounds_heuristic: owner < 0 ? null : boundsIn(buffer, graph, owner),
        ...tally([u]),
      };
    }),
  };
