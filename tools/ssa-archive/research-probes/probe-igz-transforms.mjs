...
const T = names.indexOf('igTransform'),
  G = names.indexOf('igGroup'),
  N = names.indexOf('igNode'),
  TS = names.indexOf('igTransformSequence1_5');
...
function findType(t) {
  ...
  for (let p = S1; p + 12 <= S1END; p += 4) {
    if (u32(p) === t && (u32(p + 8) >>> 24 === 1 || u32(p + 4) >>> 24 === 1)) out.push(p);
  }
  return out;
}
const trs = findType(T);
console.log('igTransform candidates:', trs.length);
...
for (const p of trs) {
  ...
  for (let q = 12; q + 64 <= 0x90; q += 4) {
    const m = [];
    for (let i = 0; i < 16; i++) m.push(f32(p + q + 4 * i));
    ...
    if (rowMajor || colMajor) {
      ...
      if (samples.length < 25 && (Math.abs(trans[0]) > 1 || Math.abs(trans[2]) > 1))
        samples.push(
          `@0x${p.toString(16)} mat@+0x${q.toString(16)} ${rowMajor ? 'row' : 'col'} T=[${trans.map(v => v.toFixed(2)).join(', ')}] name=${name}`,
        );
      break;
    }
  }
}
console.log('igTransform with matrix:', withMat);
