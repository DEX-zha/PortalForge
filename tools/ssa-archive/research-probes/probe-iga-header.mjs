...
for (const file of ['character/001_Gryphon.arc', 'character/001_Gryphon.bld', 'level/Level_027_Tutorial.arc']) {
  ...
  const entries = Array.from({ length: count }, (_, i) => ({
    start: u32(local + 12 * i),
    size: u32(local + 12 * i + 4),
    mode: u32(local + 12 * i + 8),
  }));
  ...
  console.log(
    JSON.stringify({
      ...
      tablesEnd,
      firstData,
      dataEndVsNt: dataEnd - ntLoc,
      modesByHighByte: modes,
      first3: entries.slice(0, 3).map((e, i) => ({ ...e, mode: e.mode.toString(16), name: names[i] })),
      lastName: names[count - 1],
      alignment0x800: entries.every(e => e.start % 0x800 === 0),
    }),
  );
}
