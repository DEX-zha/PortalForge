...
for(const [start,end] of [[0x80000000,0x81800000],[0x90000000,0x94000000]]) {
  ...
  for(let address=start;address<end;address+=65536) {
    ...
    while(offset!==-1){const found=address-previous.length+offset;report.matches.push('0x'+found.toString(16));console.log('MATCH 0x'+found.toString(16));offset=combined.indexOf(needle,offset+1);}
