...
try {
  ...
  if(process.argv.includes('--patch')) {
    const patch=await call('dolphin_build_patch_launch',{game,xml:path.join(local,'riivolution-smoke/smoke.xml'),patch_root:path.join(local,'riivolution-smoke'),options:[{'option-id':'mcp-smoke',choice:1}]});
  ...
  for(let i=0;i<12;i++) {
    await delay(1500);
    try {const ping=await call('dolphin_ping');console.log(JSON.stringify(ping));connected=true;break;}catch(e){console.log(e.message);}
  }
  ...
  for(const [name,args] of [
    ...
    } catch(e) {console.log(e.message);report.checks.push({name,status:'FAIL',error:e.message});process.exitCode=1;}
  }
} catch(e) {report.error=e.message;console.error(e);process.exitCode=1;}
finally {
  try {console.log(JSON.stringify(await call('dolphin_stop')));}catch{}
  await c.close();
  fs.mkdirSync(evidence,{recursive:true});
  fs.writeFileSync(path.join(evidence,process.argv.includes('--patch')?'patch-test.json':'live-test.json'),JSON.stringify(report,null,2));
}
