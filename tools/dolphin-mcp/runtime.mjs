...
export function portOccupied(port=55355) {
  return new Promise(resolve=>{
    ...
    function finish(value) {clearTimeout(timer);socket.destroy();resolve(value);}
...
export function memoryRange(address, length) {
  if (!Number.isSafeInteger(address) || !Number.isSafeInteger(length) || length < 1 || length > 65536 ||
      ![[0x80000000, 0x81800000], [0x90000000, 0x94000000]].some(([s,e]) => address >= s && address + length <= e))
...
export function bridgeCall(method, params = [], timeout = 15000, port = 55355) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(new Error('Dolphin bridge timeout: start a game and unpause the emulator. Operation outcome may be unknown; do not automatically retry writes.')), timeout);
    ...
    socket.on('data', chunk => {
      ...
      if (end !== -1) {
        try { const r = JSON.parse(data.subarray(0,end)); if (r.id !== 1) throw new Error('Invalid response ID'); r.error ? finish(new Error(r.error)) : finish(null, r.result); }
        catch(e) { finish(e); }
      }
    });
  });
...
export function buildDescriptor(game, xml, patchRoot, options) {
  return {type:'dolphin-game-mod-descriptor',version:1,'base-file':dolphinPath(existingFile(game)),
    'display-name':'PortalForge experiment',riivolution:{patches:[{xml:dolphinPath(existingFile(xml)),root:dolphinPath(patchRoot),options}]}};
}
export function initializeProfile() {
  ...
  const defaults = {
    ...
    'Dolphin.ini':'[Analytics]\nEnabled = False\nPermissionAsked = True\n[Interface]\nConfirmStop = False\nPauseOnFocusLost = False\nUsePanicHandlers = False\n[Core]\nEnableSaveStates = True\n[EmulatedUSBDevices]\nEmulateSkylanderPortal = True\n',
    'Logger.ini':'[Options]\nWriteToFile = True\nWriteToConsole = False\nVerbosity = 4\n[Logs]\nBOOT = True\nCORE = True\nDISCIO = True\nFILEMON = True\nIOS_USB = True\nScripting = True\n',
    'WiimoteNew.ini':'[Wiimote1]\nSource = 1\nExtension = Nunchuk\n[Wiimote2]\nSource = 0\n[Wiimote3]\nSource = 0\n[Wiimote4]\nSource = 0\n'
  };
  for (const [name,content] of Object.entries(defaults)) {
    const p=path.join(profile,'Config',name); if (!fs.existsSync(p)) fs.writeFileSync(p,content,{flag:'wx'});
  }
}
let child = null;
export function ownedProcess() {return child && child.exitCode === null && !child.killed ? child : null;}
export async function uiControl(operation,{file,slot=1}={}) {
  const p=ownedProcess();if(!p) throw new Error('UI controls require a Dolphin process owned by this MCP');
  const args=['-NoProfile','-File',path.join(here,'ui-control.ps1'),'-DolphinProcessId',String(p.pid),'-Operation',operation,'-Slot',String(slot)];
  let copy=null;
  if(operation==='load_figure') {
    const source=existingFile(file),data=fs.readFileSync(source);
    if(data.length!==1024) throw new Error('Expected a 1024-byte SSA Skylander dump');
    const directory=path.join(local,'figures');fs.mkdirSync(directory,{recursive:true});
    copy={source,path:path.join(directory,randomUUID()+'.sky'),sha256:createHash('sha256').update(data).digest('hex')};
    fs.writeFileSync(copy.path,data,{flag:'wx'});args.push('-Filename',copy.path);
  }
  if(operation==='save_state'||operation==='load_state') args.push('-Filename',file);
  const {stdout}=await promisify(execFile)('powershell.exe',args,{windowsHide:true,timeout:20000,encoding:'utf8'});
...
export async function launch({game, runtime='felk'}) {
  ...
  if(runtime==='felk' && await portOccupied()) throw new Error('Bridge port 55355 is already in use. Close the previous research Dolphin before launching another instance.');
  if(!game && fs.existsSync(path.join(local,'dolphin-config.json'))) game=JSON.parse(fs.readFileSync(path.join(local,'dolphin-config.json'),'utf8')).game;
  initializeProfile();
...
export async function stop() {
  const p=ownedProcess(); if (!p) return {stopped:false,reason:'No process owned by this MCP instance'};
  try { await promisify(execFile)('powershell.exe',['-NoProfile','-File',path.join(here,'close-window.ps1'),'-DolphinProcessId',String(p.pid)],{windowsHide:true,timeout:5000}); } catch {}
  for(let i=0;i<30 && p.exitCode===null && p.signalCode===null;i++) await delay(100);
  let forced=false;
  if(p.exitCode===null && p.signalCode===null) {forced=true;p.kill();await Promise.race([new Promise(r=>p.once('exit',r)),delay(5000)]);}
...
export function tailLog(lines=100) {
  ...
  try { const size=fs.fstatSync(fd).size;const start=Math.max(0,size-262144);const b=Buffer.alloc(size-start);fs.readSync(fd,b,0,b.length,start);
