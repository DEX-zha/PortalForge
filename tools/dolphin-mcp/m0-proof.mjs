...
const report={started:new Date().toISOString(),descriptor:manifest.descriptor,disc_path:manifest.disc_path,checks:[]};
...
function pass(name,detail={}) {report.checks.push({name,status:'PASS',...detail});console.log('PASS '+name+' '+JSON.stringify(detail));}
// frame_advance is bounded to 15 s upstream; boot-time loading runs below 60 fps, so wait in chunks.
async function advance(frames) {for(let done=0;done<frames;done+=120) await call('dolphin_frame_advance',{frames:Math.min(120,frames-done)});}
function logSince(offset) {
  if(!fs.existsSync(logFile)) return '';
  const size=fs.statSync(logFile).size; if(size<=offset) return '';
  const fd=fs.openSync(logFile,'r');
  try {const b=Buffer.alloc(size-offset);fs.readSync(fd,b,0,b.length,offset);return b.toString('utf8');}
...
async function bootAndObserve(label,target) {
  ...
  for(let i=0;i<30 && !connected;i++) {
    await delay(1500);
    try {await call('dolphin_ping');connected=true;}catch(e){console.log(label+' waiting bridge: '+e.message);}
  }
  ...
  for(let i=0;i<40 && !line;i++) {
    line=logSince(offset).split(/\r?\n/).find(l=>l.includes('FileMon') && l.includes('hbm/config.txt'));
    if(!line) await call('dolphin_frame_advance',{frames:60});
  }
  assert.ok(line,label+': file monitor never reported '+manifest.disc_path);
  const size=/\]:\s+([\d\s\u00a0]+kB)\s+hbm\/config\.txt/.exec(line)?.[1]?.replace(/[\s\u00a0]+/g,' ').trim();
  return {pid:launched.pid,line:line.trim(),size,log_offset:offset};
}
try {
  await c.connect(new StdioClientTransport({command:process.execPath,args:[path.join(here,'server.mjs')],stderr:'inherit'}));

  const control=await bootAndObserve('control',game);
  report.control=control;
  assert.equal(control.size,manifest.expected_original_log.split(' hbm')[0],'control run must serve the original size');
  ...
  assert.equal(patched.size,manifest.expected_patched_log.split(' hbm')[0],'patched run must serve the replacement size');
  assert.notEqual(patched.size,control.size);
  pass('patched_replacement_size',{control:control.size,patched:patched.size,replacement_bytes:manifest.replacement_size});
  ...
  report.patched.log_excerpt=logSince(patched.log_offset).split(/\r?\n/).filter(l=>/FileMon|Skylander|Portal|1430:0150/i.test(l)).slice(-40);
  report.status='PASS';
} catch(e) {report.status='FAIL';report.error=e.message;console.error(e);process.exitCode=1;}
finally {
  try {report.final_stop=JSON.parse(text(await call('dolphin_stop'))[0]);}catch(e){report.final_stop={error:e.message};}
  await c.close();
  report.finished=new Date().toISOString();
  fs.mkdirSync(evidence,{recursive:true});
  const output=path.join(evidence,'m0-proof.json');
  fs.writeFileSync(output,JSON.stringify(report,null,2));
  manifest.validation=report.status;manifest.validated_at=report.finished;manifest.report=output;
