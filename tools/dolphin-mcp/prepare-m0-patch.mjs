...
const directory = path.join(local, 'riivolution-proof');
...
const manifest = {
  ...
  replacement_size: replacement.length,
  replacement_sha256: createHash('sha256').update(replacement).digest('hex'),
  expected_original_log: '0 kB hbm/config.txt',
  expected_patched_log: Math.floor(replacement.length / 1000) + ' kB hbm/config.txt',
  disc_path: '/hbm/config.txt',
  mutation: 'append newline padding; resize=true',
  descriptor: path.join(directory, 'launch.json'),
  validation: 'PENDING',
};
