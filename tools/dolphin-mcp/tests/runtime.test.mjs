...
test('game-mod descriptor paths use forward slashes so Dolphin resolves external files',()=>{
  ...
  const d=buildDescriptor(path.join(here,'package.json'),path.join(here,'server.mjs'),here,[{'option-id':'x',choice:1}]);
...
test('RAM boundaries reject overflow, MMIO and invalid sizes',()=>{
  memoryRange(0x817ffffc,4); memoryRange(0x93fffffc,4);
  for(const [a,n] of [[0x817ffffd,4],[0x93fffffd,4],[0xcc000000,4],[0x80000000,0],[0x80000000,65537],[0x80000000,1.5]])
    assert.throws(()=>memoryRange(a,n));
});
test('valid unaligned reads remain exact byte ranges',()=>{memoryRange(0x80000001,8);});
test('bridge reconstructs fragmented responses and preserves 64-bit strings',async()=>{
  const s=createServer(socket=>socket.once('data',data=>{
  ...
  try {assert.equal(await bridgeCall('memory.write_u64',[0x80004000,'18446744073709551615'],1000,s.address().port),'18446744073709551615');}
  finally {s.close();}
});
test('silent bridge times out instead of hanging indefinitely',async()=>{
  const sockets=[];
  const s=createServer(socket=>{sockets.push(socket);socket.on('data',()=>{});});
  s.listen(0,'127.0.0.1');await once(s,'listening');
  try {await assert.rejects(bridgeCall('bridge.ping',[],50,s.address().port),/timeout/);}
  finally {sockets.forEach(x=>x.destroy());s.close();}
});
